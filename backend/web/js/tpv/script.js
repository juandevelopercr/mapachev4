...
var lang_invoice = "Factura electrónica";
var lang_invoice_s2 = "Tiquete electrónico";
...
function init() {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/get-products?value=0&barcode=' + barcodeActive,
        success: function(response) {
            var data2 = response.data.content;
            //console.log("DATA LN16 ==============>", data);
            data2 = data2.replace('prodContainer', 'prodContainer2');
            data2 = data2.replace(/class='prod'/g, "class='prod2'");
            data2 = data2.replace(/class="prod"/g, 'class="prod2"');
            $('#uno_in').append(response.data.content);
            $('#tres').append(data2.content);
            mmargin();
            mmargin2();
...
function loadProducts(catId) {
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/get-products?value=' + catId + '&barcode=' + barcodeActive,
        success: function(response) {
            //console.log("DATA LN63 ==============>", data);
            data2 = response.data.content;
            data2 = data2.replace("id='prodContainer'", "id='prodContainer2'");
            data2 = data2.replace(/class='prod'/g, "class='prod2'");
            data2 = data2.replace(/class="prod"/g, 'class="prod2"');
            $('#prodContainer').replaceWith(response.data.content);
            $('#tres').html(data2);
            mmargin();
            mmargin2();
            last_load = catId;
 ...
 */
function addProduct(productId, name, price, percent_iva, discount_amount, exoneration, nature_discount, options, mvw) {
    ...
    if (ifexr == 0) {
        var quantity = 1;
        var discount = discount_amount * quantity;
        var subtotal = price * quantity - discount;
        var iva = subtotal * (percent_iva / 100);
        var exonerated = iva * (exoneration / 100);
        subtotal = subtotal + iva - exonerated;

        $('#orderTable').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm_' + mynm + '">1</td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');


        $('#orderTable2').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm2_' + mynm + '">1</td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');
...
function addProduct2(productId, name, price, percent_iva, discount_amount, exoneration, nature_discount, options, mvw) {
    ...
    if (ifexr == 0) {
        ...
        $('#orderTable').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm_' + mynm + '">1</td>' + '<td>' + '</td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');

        $('#orderTable2').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm2_' + mynm + '">1</td>' + '</td>' + '<td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');
        $("#order").animate({
            scrollTop: $('#order')[0].scrollHeight
        }, 1000);
        $("#order2").animate({
...
function addProduct_b(productId, name, price, percent_iva, discount_amount, exoneration, nature_discount, options, mvw) {
    //*//console.log("ADDPRODUCT-productId, name, price, options, mvw- ==> ", productId + " | " + name + " | " + price + " | " + options + " | " + mvw);
    if (mvw = 1) {
        mmd_closed();
    }
    ...
    if (ifexr == 0) {
        ...
        $('#orderTable').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm_' + mynm + '">1</td>' + '<td>' + '<td>' + format(discount) + '</td>' + '<td>' + format(iva) + '</td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');

        $('#orderTable2').append('<tr id="orderRow_' + productId + '" data-percent_iva="' + percent_iva + '" data-discount_amount="' + discount_amount + '" data-exoneration="' + exoneration + '" data-nature_discount="' + nature_discount + '">' + '<td style="text-align:left;padding-left: 6px;">' + name.replace(/&quot;/i,
            "'") + '</td>' + '<td>' + format(price) + '</td>' + '<td id="nm2_' + mynm + '">1</td>' + '<td>' + '<td>' + format(discount) + '</td>' + '<td>' + format(iva) + '</td>' + '<td>' + format(subtotal) + '</td>' + '<td>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'appendProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-plus"></span></button>' + '<button type="button" class="btn btn-primary btn-xs" onclick=\'removeProduct("' + mynm + '"); return false;\'><span class="glyphicon glyphicon-minus"></span></button>' + '</td>' + '</tr>');
        $("#order").animate({
            scrollTop: $('#order')[0].scrollHeight
        }, 1000);
        $("#order2").animate({
...
function calculateTotal() {
    ...
    $('#orderTable tr').each(function() { // iterate table rows
        if (rows) {
            ...
            //Adicionado por caceres
            var discount = $(this).data("discount_amount");
            discount = discount * quantity;
            var percent_iva = $(this).data("percent_iva");
            var exoneration = $(this).data("exoneration");
            var percent_iva = $(this).data("percent_iva");
            ...
            var iva = subtotal * (percent_iva / 100);
            var exonerated = iva * (exoneration / 100);
            var rowtotal = subtotal + iva - exonerated;

            // Totales
            total_subtotal = total_subtotal + subtotal;
            total_discount = total_discount + discount
            total_iva = total_iva + iva;
            total_exonerated = total_exonerated + exonerated;
            total_total = total_total + subtotal - discount + iva - exonerated;
            ...
            $($('#orderTable2').find('tr')[rowIndex]).children('td')[2].innerHTML = quantity;
            //$($('#orderTable2').find('tr')[rowIndex]).children('td')[3].innerHTML = format(discount);
            //$($('#orderTable2').find('tr')[rowIndex]).children('td')[4].innerHTML = format(iva);
            $($('#orderTable2').find('tr')[rowIndex]).children('td')[3].innerHTML = format(subtotal);

            //}
            //console.log("ADDVALUE ===432=calculateTotal===> ", addValue);
            //console.log("FORMAT---ADDVALUE ===433=calculateTotal===> ",format(addValue));
            //$(this).find("td").eq(3).html(format(discount));
            //$(this).find("td").eq(4).html(format(iva));
            $(this).find("td").eq(3).html(format(subtotal));
    ...
    $("#total_discount").html(format(total_discount));
    $("#total_subtotal").html(format(total_subtotal));
    $("#total_tax").html(format(total_iva));
    $("#total_exonerate").html(format(total_exonerated));
    $("#total_price").html(format(total_total));

    $("#total_discount2").html(format(total_discount));
    $("#total_subtotal2").html(format(total_subtotal));
    $("#total_tax2").html(format(total_iva));
    $("#total_exonerate2").html(format(total_exonerated));
    $("#total_price2").html(format(total_total));
...
function add_customer() {
    ...
    strVar += '<form onsubmit="customerAdd(); return false;" id="md_frm1" class="form-vertical">\n';
    strVar += '<div class="row mdal ln-s">\n';
    strVar += '<div class="col-xs-4 col-md-2">Buscar cliente</div>\n';
    strVar += '<div class="col-xs-8 col-md-4">\n';
    strVar += '<input type="text" id="customern" class="mdl" required value="" style="width: 100% !important;">\n';
    strVar += '</div>\n';
    strVar += '<div class="col-xs-4 col-md-2 text-right"></div>\n';
    strVar += '<div class="col-xs-8 col-md-4 text-right">\n';
    strVar += '<button type="submit" class="btn btn-primary">Seleccionar Cliente</button>\n';
    strVar += '</div>\n';
    strVar += '</div>\n';
    strVar += '</form>\n';
    */
    $.ajax({
        url: '/tpv/ajax/customerform',
    ...
    strVar += 'style="width: 100%;" required>';
    strVar += '</div>';

    strVar += '<div class="col-xs-4 col-md-2">Tipo Identificación</div>\n';
    strVar += '<div class="col-xs-8 col-md-4 text-right">\n';
    strVar += '<select name="tipo_identificacion" class="mdl" id="customerTipo" required>\n';
    strVar += '<option value="6" selected>Cédula Física</option>\n';
    strVar += '<option value="7">Cédula Jurídica</option>\n';
    strVar += '</select>\n';
    strVar += '</div>\n';
    strVar += '</div>\n';
    strVar += '<div class="row mdal">\n';
    strVar += '<div class="col-xs-4 col-md-2">Identificación</div>\n';
    strVar += '<div class="col-xs-8 col-md-4">\n';
    strVar += '<input type="text" id="customerIdentificacion" class="mdl" value="" required>\n';
    strVar += '</div>\n';

    strVar += '<div class="col-xs-4 col-md-2">Correo electrónico</div>\n';
    strVar += '<div class="col-xs-8 col-md-4">\n';
    strVar += '<input type="text" id="customeremail" class="mdl" value="" required>\n';
    strVar += '</div>\n';
    ...
    strVar += "            $.ajax({\n";
    strVar += "                url: '/tpv/ajax/searchcustomer?term=' + myf,\n";
    strVar += "                type: 'POST',\n";
...
function customerAdd() {
    ...
    //var mtitle = myarr[0].trim();
    var temp = mtitle.split("-");
    var mtitle = temp[0].trim();
    //console.log("myarr ==> ", myarr)
    ...
    //console.log("mid ==> ", mid);
    $.ajax({
        url: '/tpv/ajax/search_client_name?id=' + mid + "&name=" + mtitle,
...
function customerAddNew() {
    ...
    $.ajax({
        ...
        url: '/tpv/ajax/addnewcustomer?name=' + $('#customerFirstname').val() + '&tipo_identificacion=' + $('#customerTipo').val() + '&identificacion=' + $('#customerIdentificacion').val(),
        success: function(response) {
            if (response.data.result == false) {
                ...
                $('#orderCustomerId').val(response.data.customer_id);
...
function refund() {
    ...
    if (url > 0) {
        myno = "";
        $.ajax({
            url: '/tpv/ajax/purchasereturn?value=' + url,
            //url: 'ajax.php?action=purchasereturn&value=' + url,
            success: function(data) {
                html = data.body;
                title = lang_return;
                if (data.devuelta == true) {
                    myno = " disabled";
...
function refundfinish2() {
    ...
    */
    ...
    $.ajax({
        url: '/tpv/ajax/purchasereturn2?value=' + mres,
        ...
        success: function(data) {
            if (data.result == false) {
                ...
                if (mtd_print == 'Fac' || mtd_print == 'FacSimpli') {
                    ...
                    //$('#mi_print').print();
                    $("#mi_print").print({
                        globalStyles: true, // Si se debe incluir el estilo del documento principal, el valor predeterminado es verdadero
                        mediaPrint: false, // Si se debe incluir la etiqueta del enlace de media = 'print'. Será anulado por la opción globalStyles, el valor predeterminado es falso
                        stylesheet: null, // La dirección URL de la hoja de estilo externa, el valor predeterminado es nulo

                        noPrintSelector: ".no-print", // Selector de JQuery para elementos que no se quieren imprimir, el valor predeterminado es ".no-print"
                        iframe: true, // Si se usa un iframe para reemplazar la ventana emergente del formulario de impresión, verdadero significa imprimir en esta página, falso significa abrir una nueva página para imprimir, el valor predeterminado es verdadero
                        append: null, // Agrega el contenido al reverso del contenido impreso
                        prepend: null, // Agregue el contenido al frente del contenido impreso, que se puede usar como el contenido que se imprimirá
                        deferred: $.Deferred() //Llamar de vuelta
                    });
                    $('#mi_print').empty();
                    mfocus_search();
                    //}
...
function factur() {
    ...
    if (order_imprent > 0) {
        $("#spinner").show();
        $.ajax({
            url: '/tpv/ajax/tip_factur?value=' + order_imprent,
            ...
            success: function(data) {
                ...
                html += '            <td><input type=\"text\" id=\"idreturn\" value=\"' + num_fac + '\" size=\"30\"></td>\n';
...
function outofbox() {
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/checkstatusbox',
        dataType: "json",
        success: function(data) {
            $("#spinner").hide();

            if (data.status == 'CERRADA') {
                html = "La caja está cerrada. Para adicionar efectivo abra la caja e inténtelo de nuevo";
...
function outofboxfinish() {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/outofbox?value=' + cati + '&coment=' + descrip,
...
function openbox() {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/openbox',
        dataType: "html",
        success: function(data) {
            strVar = data;
            html_d = strVar;
            title = 'Apertura de Caja'; //lang_discount;
            mfoot = mfoot = '';
            width = "3";
            mtype = '';
            $("#spinner").hide();
...
function entryinboxfinish() {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/entryinbox?value=' + cati + '&coment=' + descrip,
    ...
    /*
    $.ajax({
        url: '/tpv/ajax/lst_box_operations',
        //url: 'ajax.php?action=lst_box_operations',
        success: function(data) {
            $('#uno_in').html(data);
            $('#tres').html(data);
...
function logout_app() {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/logout_app?value=0',
...
function logout() {
    ...
    $.ajax({
        type: "POST",
        //url: '/tpv/ajax/closeboxfinish?value=0',
        url: '/site/logout',
    ...
    if (cuent > 1) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.status == 'CERRADA') {
                    html = "La caja está cerrada. Para facturar abra la caja e inténtelo de nuevo";
                    title = lang_entryinbox_ttl;
                    mfoot = '';
                    width = "1";
                    mtype = '';
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            mmd(data, title, mfoot, "3", "");
                            mivl = $("#orderTotal2").html();
                            $("#PMAmount").val(mivl);
                            $("#HPMAmount").val(mivl);
                            //$("#PMAmountPendiente").val(mivl);
                            ...
                            /*
                            $("#PMValue").focusout(function(event) {
                                var tx = $(this);
                                if (tx.val().length == 2) {

                                    console.log("Consola DDDD");
...
function checkoutAddPayment() {
    ...
    if (clean_number($("#HPMAmount").val(), true) >= 0) {
        var current = $('#paymentLines').val();
        //console.log("CURRENT ==1717===> ", current)
        if (current.length > 0) current += "|";
        var lineAddition = $('input:radio[name=PM]:checked').val() + ":" + clean_number($("#HPMValue").val(), true);
    ...
    //alert(mpay);
    if (mpay == 1) {
        var vueltas = $("#HPMValue").val();
        vueltas = vueltas.replace(" ", "");
        $("#HPMValue").val(vueltas);
        if (clean_number($("#HPMAmount").val(), true) >= 0) {
            var current = $('#paymentLines').val();
            //console.log("CURRENT ==1717===> ", current)
            if (current.length > 0) current += "|";
        ...
        current = clean_number($("#HPMAmount").val(), true);
        console.log("current  ==1724==> ", current);
        var replic = clean_number($("#HPMValue").val(), true);
        current -= replic;
        //console.log("replic  ==1727==> ", replic)
        current = clean_number(current, true);
        $("#PMValue").val(clean_number(current, true));
        $("#HPMValue").val(clean_number(current, true));
        $("#HPMAmount").val(clean_number(current, true));
        ...
        if (current > 0)
            $("#PMAmountPendiente").val(clean_number(Math.abs(current), true));
        else
            $("#PMValue").val(0);

        if (current < 0)
            $("#PMVuelto").val(clean_number(Math.abs(current), true));
        else
            $("#PMVuelto").val(clean_number(0, true));

    } else
    if (mpay == 2) { // Tarjeta
        /*
        if ($("#PMTarjeta").val().length == 0)
            alert("Entre el número de tarjeta");
        else        
        if ($("#PMTarjeta").val().length < 4)
            alert("Deben introducir 4 dígios de la tarjeta");
        else
        if ($("#PMTarjeta").val().length > 4)
            alert("Deben introducir exactamente 4 dígios de la tarjeta");
        else
        */
        if ($("#PMTarjetaReferencia").val().length == 0)
            alert("Entre la referencia de la tarjeta");
        else
            mrst = 0;
    } else
    if (mpay == 3) { // Cheque
        if ($("#PMBanco").val().length == 0)
            alert("Seleccione el banco");
        else
        if ($("#PMBancoCheque").val().length == 0)
            alert("Entre número de cheque");
        else
            mrst = 0;
    } else
    if (mpay == 4) { // Transferencia
        if ($("#PMBanco").val().length == 0)
            alert("Seleccione el banco");
        else
        if ($("#PMBancoComprobante").val().length == 0)
            alert("Entre comprobante");
        else
            mrst = 0;
    }
...
function checkoutFinish(tipo_factur) {
    ...
    */
    ...
    //pago con tarjeta
    tarjeta = $("#PMTarjeta").val();
    tarjeta_referencia = $("#PMTarjetaReferencia").val();

    banco = $("#PMBanco").val();
    banco_cheque = $("#PMBancoCheque").val();
    banco_comprobante = $("#PMBancoComprobante").val();

    $("#spinner").show();
    $.ajax({
        //url: 'ajax.php?action=createOrder&value=' + url,
        type: "POST",
        url: '/tpv/ajax/create_order',
        data: {
            ordenes: url,
            tipo: tipo_factur,
            lines: $('#paymentLines').val(),
            customerId: $('#orderCustomerId').val(),
            mpay: mpay,
            yourpay: vueltas,
            tarjeta: tarjeta,
            tarjeta_referencia: tarjeta_referencia,
            banco: banco,
            banco_cheque: banco_cheque,
            banco_comprobante: banco_comprobante,
        },
        success: function(data) {
            if (data.result == false)
                alert('No se ha podido crear la factura. Póngase en contacto con el administrador del sistema');
            //console.log("DESDE SERVER:CREATE ORDER- ORDERID ==1638====> ",data);
            title = lang_orderAdd; //lang_payment;
            mfoot = "<button type='button' class='btn btn-default' data-dismiss='modal'>Cerrar</button>";
            mmd(lang_orderAddT, title, mfoot, "1", "");
            setTimeout('mmd_closed()', 1400);
            ...
            //CACERES
            $('#orderCustomerId').val(data.customer_id);
            $('#orderCustomerName').val(data.customer_name);
            $('#orderCustomer').html(data.customer_name);
            ...
            if (facturax == 1) {
                ...
                if (tipo_factur == "Fac" || tipo_factur == "fac") { //Factura
                    printReceiptPopup(data.invoice_id, committed, mg);
                } else { //Factura simplificada	
                    // //console.log("ENTRO LINEA 1702");
                    if (metodo == '0') {
                        printReceiptPopup2b(data.invoice_id, committed, mg);
                    } else {
                        printReceiptPopup3c(data.invoice_id, committed, mg);
                    }
                }
...
function printReceiptPopup(orderId, committed, mg) {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/invoice?orderId=' + orderId + '&committed=' + committed + "&mg=" + mg,
        //url: 'receipt.php' + mv,
        dataType: "html",
        success: function(data) {
            $('#mi_print').empty();
...
function printReceiptPopup2b(orderId, committed, mg) {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/tiquete_html?orderId=' + orderId + '&committed=' + committed + "&mg=" + mg,
        //url: 'receipt2.php' + mv,
        dataType: "html",
        success: function(data) {
...
function newSearch(miv) {
    ...
    if (res_p == "ini") {
        ...
        $.ajax({
            ...
            url: '/tpv/ajax/search-products?value=' + miv,
            success: function(response) {
                ...
                if (response.data.id > 0) {
                    ...
                    product_percent_iva = response.data.percent_iva;
                    product_discount_amount = response.data.discount_amount;
                    product_exoneration = response.data.exoneration;
                    product_nature_discount = response.data.nature_discount;
                    addProduct(product_id, product_name, product_price, product_percent_iva, product_discount_amount, product_exoneration, product_nature_discount, 0, 0);
                    //addProduct(product_id, product_name, product_price, 0, 0);
                }
                $("#search_in").val('');
                $("#spinner").hide();
            }
...
function mnu_up_right() {
    ...
    if (res_p == "ini") {
        ...
        var strVar = "";
        strVar += "                 <a data-toggle=\"modal\" href=\"#\" class=\"btn btn-default\" onClick=\"openbox();\"  title=\"Abrir caja\" > ";
        strVar += "						<span class=\"glyphicon glyphicon-lock\"><\/span>&nbsp;&nbsp;Abrir caja";
        strVar += "					<\/a>";
...
function m_historicoventas() {
    window.location.href = "/invoice-tpv/index";
}
...
function display_orde($nm) {
    ...
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/display_order?order_id=' + $nm,
...
function read_order() {
    ...
    xmorder = JSON.stringify(xmorder);
    //xmorder = JSON.stringify(xmorder);
    ...
    if (mnm > 1) {
        ...
        $.ajax({
            type: "POST",
            url: '/tpv/ajax/save_preorder',
            data: { mprop: mprop, xmorder: xmorder },
            success: function(response) {
...
function see_pre_order(nm) {
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/view_pre_order?value=' + nm,
...
function del_order() {
    ...
    $("#total_discount").html(format(0));
    $("#total_subtotal").html(format(0));
    $("#total_tax").html(format(0));
    $("#total_exonerate").html(format(0));
    $("#total_price").html(format(0));

    $("#total_discount2").html(format(0));
    $("#total_subtotal2").html(format(0));
    $("#total_tax2").html(format(0));
    $("#total_exonerate2").html(format(0));
    $("#total_price2").html(format(0));
...
function del_pre_lst(nm) {
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/del_db_pre_order?value=' + nm,
        success: function(data) {
            $("#f-" + nm).remove();
            $("#spinner").hide();
        }
...
function display_status_box() {
    $("#spinner").show();
    $.ajax({
        url: '/tpv/ajax/boxstatus?value=0',
        //url: 'ajax.php?action=boxstatus&value=0',
        success: function(data) {
...
function dir_ini() {
    window.location.href = "/tpv/invoice/index";
    $("#mpantalla").val("ini");
}
...
// CREADO POR CACERES PARA FILTRAR LA BUSQUEDA
function renderFileter() {
