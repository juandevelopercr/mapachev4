...
/**
 * This is the model class for table "payment_method_has_payment_order".
 *
 ...
 */
class PaymentMethodHasPaymentOrder extends \yii\db\ActiveRecord
{
    ...
    {
        return 'payment_method_has_payment_order';
    }
    ...
    {
        return [
            [['payment_order_id', 'payment_method_id'], 'required'],
            [['payment_order_id', 'payment_method_id'], 'default', 'value' => null],
            [['payment_order_id', 'payment_method_id'], 'integer'],
            [['payment_order_id', 'payment_method_id'], 'unique', 'targetAttribute' => ['payment_order_id', 'payment_method_id']],
    ...
    {
        return [
            'payment_order_id' => Yii::t('backend', 'Payment Order ID'),
    ...
    */
    ...
    {
        return "/payment-method-has-payment-order";
    }
    ...
    */
     ...
     * @return bool
     */
    public static function addRelation($payment_method_id, $payment_order_id)
    {
        $model= new PaymentMethodHasPaymentOrder();
        $model->payment_order_id = $payment_order_id;
     ...
     */
    public static function deleteRelation($payment_method_id, $payment_order_id)
    {
        $model= self::find()->where(['payment_order_id' => $payment_order_id, 'payment_method_id' => $payment_method_id])->one();
     ...
     * @param $payment_order_id
     * @param bool $as_array
     * @return array|PaymentMethodHasPaymentOrder[]|\yii\db\ActiveRecord[]
     */
    public static function getPaymentMethodByPaymentOrderId($payment_order_id,$as_array = true)
    {
        $query= self::find()
     ...
     * @return string
     */
    public static function getPaymentMethodStringByPaymentOrder($id)
    {
        $payment_method = self::find()->where(['payment_order_id'=>$id])->one();
        $result = '';

        if($payment_method !== null)
        {
            $relations = self::getPaymentMethodByPaymentOrderId($id,false);
            $array = [];
     ...
     * @return array
     */
    public static function getItemsAsignedByPaymentOrderId($id)
    {
        $items_assigned = self::getPaymentMethodByPaymentOrderId($id);

        $items_ids= [];
        foreach ($items_assigned as $value)
        {
            $items_ids[]= $value['payment_order_id'];
        }
