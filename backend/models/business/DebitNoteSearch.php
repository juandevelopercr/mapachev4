...
/**
 * DebitNoteSearch represents the model behind the search form of `backend\models\business\DebitNote`.
 */
class DebitNoteSearch extends DebitNote
{
    ...
    {
        return [
            [['id', 'branch_office_id', 'customer_id', 'credit_days_id', 'condition_sale_id', 'currency_id', 'status', 
              'status_hacienda', 'route_transport_id', 'debit_note_type', 'box_id'], 'integer'],
    ...
    {
        $query = DebitNote::find()->join('INNER JOIN', 'branch_office', 'debit_note.branch_office_id = branch_office.id')
                                  ->join('INNER JOIN', 'boxes', "debit_note.box_id = boxes.id AND boxes.is_point_sale = ".$is_point_sale."")
                                  ->join('INNER JOIN', 'seller_has_debit_note', "seller_has_debit_note.debit_note_id = debit_note.id")
                                  ->join('LEFT JOIN', 'collector_has_debit_note', "collector_has_debit_note.debit_note_id = debit_note.id");  
        ...
        {
            if (isset($_REQUEST['DebitNoteSearch']['sellers']) && !empty($_REQUEST['DebitNoteSearch']['sellers'])){
                $query->join('INNER JOIN', 'seller_has_debit_note', "seller_has_debit_note.debit_note_id = debit_note.id");
                $query->where(['seller_has_debit_note.seller_id' => $_REQUEST['DebitNoteSearch']['sellers']]); 
            }

            if (isset($_REQUEST['DebitNoteSearch']['collectors']) && !empty($_REQUEST['DebitNoteSearch']['collectors'])){
                $query->join('INNER JOIN', 'collector_has_debit_note', "collector_has_debit_note.debit_note_id = debit_note.id");
                $query->where(['collector_has_debit_note.collector_id' => $_REQUEST['DebitNoteSearch']['collectors']]); 
            }              
        } 
        */  
        ...
        */
        ...
        $query->andFilterWhere([
            ...
            'debit_note_type' => $this->debit_note_type,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
            'box_id'=> $this->id,
            'total_comprobante'=> $total_comprobante,
            'seller_has_debit_note.seller_id'=> $this->sellers,
            'collector_has_debit_note.seller_id'=> $this->collectors 
        ]);
