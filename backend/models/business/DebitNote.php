 ...
 * @property int|null $debit_note_type
 ...
 * @property int|null $correct_debit_note 1 Si corrige una factura
 * @property int|null $correct_debit_note_id id de la factura que corrige
 ...
 * @property float|null $total_tax_crc
 * @property float|null $total_tax_usd
 * @property float|null $total_discount_crc
 * @property float|null $total_discount_usd
 * @property float|null $total_proof_crc
 * @property float|null $total_proof_usd
 ...
 * @property CreditDays $debitDays
 * @property Currency $currency
 * @property Customer $customer
 * @property DebitNote $correctDebitNote
 * @property DebitNote[] $debit_notes
 * @property User $seller
 * @property User $collector
 * @property ItemDebitNote[] $itemDebitNotes
 * @property PaymentMethodHasDebitNote[] $paymentMethodHasDebitNotes
...
class DebitNote extends BaseModel
{
    ...
    {
        return [
            [['emission_date', 'consecutive', 'branch_office_id', 'customer_id', 'condition_sale_id', 'currency_id', 'status', 'change_type', 'payment_methods','debit_note_type'], 'required'],
            [['branch_office_id', 'customer_id', 'condition_sale_id', 'credit_days_id', 'currency_id', 'status_account_receivable_id', 'contingency', 
              'correct_debit_note', 'correct_debit_note_id', 'erased_by_note', 'num_request_hacienda_set', 'num_request_hacienda_get', 'status', 'debit_note_type',
              'status_hacienda', 'route_transport_id', 'ready_to_send_email', 'email_sent', 'box_id'], 'integer'],
            ...
            [['correct_debit_note_id'], 'exist', 'skipOnError' => true, 'targetClass' => DebitNote::className(), 'targetAttribute' => ['correct_debit_note_id' => 'id']],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'debit_note_type' => Yii::t('backend', 'Tipo'),
            ...
            'correct_debit_note' => Yii::t('backend', 'Corrige factura'),
            'correct_debit_note_id' => Yii::t('backend', 'Factura que corrige'),
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getCorrectDebitNote()
    {
        return $this->hasOne(DebitNote::className(), ['id' => 'correct_debit_note_id']);
    }
    ...
    {
        return $this->hasMany(DebitNote::className(), ['correct_debit_note_id' => 'id']);
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getItemDebitNotes()
    {
        return $this->hasMany(ItemDebitNote::className(), ['debit_note_id' => 'id']);
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getPaymentMethodHasDebitNotes()
    {
        return $this->hasMany(PaymentMethodHasDebitNote::className(), ['debit_note_id' => 'id']);
    }
    ...
    {
        return $this->hasMany(PaymentMethod::className(), ['id' => 'payment_method_id'])->viaTable('payment_method_has_debit_note', ['debit_note_id' => 'id']);
    }
    ...
    */
    ...
    public function generateConsecutive()
    {
        ...
        $c_number = '02'; //Nota de debito

        $identificacion = $a_number . $b_number . $c_number;

        $connection = \Yii::$app->db;
        $sql = "SELECT MAX(SUBSTRING(consecutive, 11, 10)) AS consecutive FROM debit_note where SUBSTRING(consecutive, 1, 10) = '" . $identificacion . "'";
    ...
    {
        //$resume = self::getResumeDebitNote($this->id);
    ...
    {
        $moneda = 'COLONES';
        $factura_detalles = ItemDebitNote::findAll(['debit_note_id' => $this->id]);
    ...
    {
        return ItemDebitNote::find()->where(['debit_note_id' => $id])->sum('price_total');
    }

    public static function getResumeDebitNote($debit_note_id)
    {
        $resume = ItemDebitNote::find()
            ->select([
    ...
    {
        $subject = Yii::t('backend','NotaDebito #'.$this->consecutive);

        $mailer = Yii::$app->mail->compose(['html' => 'debit_note-html'], ['debit_note_number' => $this->consecutive])
            ->setTo($this->customer->email)
            ->setFrom([Setting::getEmail() => Setting::getName()])
            ->setSubject($subject)
            ->attach($file_pdf, ['fileName'=>'NotaDebito_'.$this->consecutive]);
    ...
    {
        $unit_type = '';
        $items = ItemDebitNote::find()->where(['debit_note_id' => $this->id])->all();
        foreach ($items AS $key => $item)
        {
            if(isset($item->product_id))
            {
                ...
                {
                    DebitNote::sendAlertStock($current_stock, $item->quantity, $request_quantity_total, $product, $this, $unit_type);
                }
            }
        }
    }
     ...
     * @param $email
     */
    public static function sendAlertStock($current_stock, $request_quantity, $request_quantity_total, $product, $debit_note, $unit_type = '')
    {
        ...
        if($cc_mails !== '' && GlobalFunctions::validateCCMails($cc_mails))
        {
            ...
            $mailer = Yii::$app->mail->compose(['html' => 'alert_debit_note_stock-html'], ['current_stock' => $current_stock, 'request_quantity' => $request_quantity, 'request_quantity_total' => $request_quantity_total, 'debit_note' => $debit_note,'product' => $product, 'unit_type' => $unit_type])
                ->setTo($mails_to_send)
                ->setFrom([Setting::getEmail() => Setting::getName()])
                ->setSubject($subject);
     ...
     * @param $new_status
     */
    public static function setStatusHacienda($debit_note_id, $new_status)
    {
        $debit_note = self::findOne($debit_note_id);
        $debit_note->status_hacienda = $new_status;
    ...
    {
        if(!file_exists("uploads/debit_note_qr/") || !is_dir("uploads/debit_note_qr/")){
            try{
                FileHelper::createDirectory("uploads/debit_note_qr/", 0777);
            }catch (\Exception $exception){
                Yii::info("Error handling debit_note_qr folder resources");
            }
        ...
        $path = Yii::$app->basePath . '/web/uploads/debit_note_qr/';

        $qrCode->writeFile($path . $name_qrcode . '.png');

        $file = GlobalFunctions::BASE_URL. Url::to('@web/uploads/debit_note_qr/' . $name_qrcode . '.png');
    ...
    public static function verifyResponseStatusHacienda($key, $status_hacienda, $xml_response_hacienda_decode)
    {
        if($key !== null)
        {
            $debit_note = self::find()->where(['key' => $key])->one();
            ...
            if($debit_note !== null)
            {
                ...
                {
                    self::setStatusHacienda($debit_note->id, UtilsConstants::HACIENDA_STATUS_REJECTED); // Rechazada

                    $xml_filename = 'ND-MH-'.$debit_note->key.'.xml';
                    $path = Yii::getAlias('@backend/web/uploads/xmlh/'.$xml_filename);
                    file_put_contents($path, $xml_response_hacienda_decode);
                    $debit_note->response_xml = $path;
                }
                elseif ($status_hacienda == 'aceptado')
                {
                    self::setStatusHacienda($debit_note->id, UtilsConstants::HACIENDA_STATUS_ACCEPTED); // Aceptada

                    $xml_filename = 'ND-MH-'.$debit_note->key.'.xml';
                    $path = Yii::getAlias('@backend/web/uploads/xmlh/'.$xml_filename);
                    file_put_contents($path, $xml_response_hacienda_decode);
                    $debit_note->response_xml = $xml_filename;
                }
                elseif ($status_hacienda == 'recibido')
                {
                    self::setStatusHacienda($debit_note->id, UtilsConstants::HACIENDA_STATUS_RECEIVED); // Recibida
                }
