...
/**
 * This is the model class for table "item_credit_note".
 *
 * @property int $id
 ...
 */
class ItemCreditNote extends BaseModel
{
    const UPDATE_TYPE_ADD = 1;
    const UPDATE_TYPE_MINUS = 2;   
    public $quantity_devuelta; 
    ...
    public function rules()
    {
        return [
            ...
            [['credit_note_id', 'product_id', 'service_id', 'user_id', 'price_type','unit_type_id'], 'default', 'value' => null],
            [['credit_note_id', 'product_id', 'service_id', 'user_id', 'price_type','unit_type_id', 'tax_type_id', 'tax_rate_type_id', 'exoneration_document_type_id'], 'integer'],
            [['quantity', 'price_unit', 'subtotal', 'tax_amount', 'discount_amount', 'exonerate_amount', 'price_total', 'quantity_devuelta'], 'number'],
    ...
    public function attributeLabels()
    {
        return [
            'id' => 'ID',
            'credit_note_id' => Yii::t('backend', 'Nota de crédito'),
            ...
            'quantity_devuelta' => Yii::t('backend', 'Cantidad a devolver'),           
        ];
    }
    ...
    */
    ...
    public function fields()
    {
        ...
        $fields['id'] = 'id';
        $fields['credit_note_id'] = 'credit_note_id';
        $fields['code'] = 'code';
        $fields['description'] = 'description';

        $fields['product_id'] = 'product_id';
        $fields['product_label'] = function(ItemCreditNote $model){
            return isset($model->product_id)? $model->product->description : '';
        };

        $fields['service_id'] = 'service_id';
        $fields['service_label'] = function(ItemCreditNote $model){
        ...
        $fields['price_type_label'] = function(ItemCreditNote $model){
        ...
        $fields['unit_type_label'] = function(ItemCreditNote $model){
            return isset($model->unit_type_id)? $model->unitType->code : '';
        };

        $fields['user_id'] = 'user_id';
        $fields['user_label'] = function(ItemCreditNote $model){
    ...
    public function getQuantityNotaActual()
    {
        $quantity = 0;
        $quantity_rest = 0;
        // obtener la nota de credito y buscar la factura mediante reference_number
        $nota = $this->creditNote;
        $invoice = Invoice::find()->where(['key'=>trim($nota->reference_number)])->one();
        if (!is_null($this->product_id && $this->product_id > 0))
            $item = ItemInvoice::find()->where(['invoice_id'=>$invoice->id, 'product_id'=>$this->product_id])->one();
        else
        if (!is_null($this->service_id && $this->service_id > 0))
            $item = ItemInvoice::find()->where(['invoice_id'=>$invoice->id, 'service_id'=>$this->service_id])->one(); 

        if (!is_null($item))            
            $quantity = $item->quantity;
        
        // Buscar las notas de crédito diferente a esta
        $status = [UtilsConstants::HACIENDA_STATUS_NOT_SENT, UtilsConstants::HACIENDA_STATUS_RECEIVED, UtilsConstants::HACIENDA_STATUS_ACCEPTED];
        $lastNotaCredito = CreditNote::find()->where(['reference_number'=>$invoice->key])
                                             ->andWhere(['status_hacienda'=>$status])
                                             ->orderBy('id DESC')->one();
        if (!is_null($lastNotaCredito) && $lastNotaCredito->id != $nota->id){
            if (!is_null($this->product_id && $this->product_id > 0))
                $itemNote = ItemCreditNote::find()->where(['credit_note_id'=>$lastNotaCredito->id, 'product_id'=>$this->product_id])->one();
            else
            if (!is_null($this->service_id && $this->service_id > 0))                
                $itemNote = ItemCreditNote::find()->where(['credit_note_id'=>$lastNotaCredito->id, 'service_id'=>$this->service_id])->one();
            
            if (!is_null($itemNote)){
                $quantity_rest = $itemNote->quantity;  
