...
/**
 * This is the model class for table "payment_method_has_debit_note".
 *
 * @property int $debit_note_id
 * @property int $payment_method_id
 *
 * @property PaymentMethod $paymentMethod
 * @property DebitNote $debit_note

 */
class PaymentMethodHasDebitNote extends \yii\db\ActiveRecord
{
    ...
    {
        return 'payment_method_has_debit_note';
    }
    ...
    {
        return [
            [['debit_note_id', 'payment_method_id'], 'required'],
            [['debit_note_id', 'payment_method_id'], 'default', 'value' => null],
            [['debit_note_id', 'payment_method_id'], 'integer'],
            [['debit_note_id', 'payment_method_id'], 'unique', 'targetAttribute' => ['debit_note_id', 'payment_method_id']],
    ...
    */
    ...
    {
        return "/payment-method-has-debit-note";
    }
    ...
    */
     ...
     * @return bool
     */
    public static function addRelation($payment_method_id, $debit_note_id)
    {
        $model= new PaymentMethodHasDebitNote();
        $model->debit_note_id = $debit_note_id;
     ...
     */
    public static function deleteRelation($payment_method_id, $debit_note_id)
    {
        $model= self::find()->where(['debit_note_id' => $debit_note_id, 'payment_method_id' => $payment_method_id])->one();
     ...
     * @param $debit_note_id
     * @param bool $as_array
     * @return array|PaymentMethodHasDebitNote[]|\yii\db\ActiveRecord[]
     */
    public static function getPaymentMethodByDebitNoteId($debit_note_id,$as_array = true)
    {
        $query= self::find()
     ...
     * @return string
     */
    public static function getPaymentMethodStringByDebitNote($id)
    {
        $payment_method = self::find()->where(['debit_note_id'=>$id])->one();
        $result = '';

        if($payment_method !== null)
        {
            $relations = self::getPaymentMethodByDebitNoteId($id,false);
            $array = [];
     ...
     * @return array
     */
    public static function getItemsAsignedByDebitNoteId($id)
    {
        $items_assigned = self::getPaymentMethodByDebitNoteId($id);

        $items_ids= [];
        foreach ($items_assigned as $value)
        {
            $items_ids[]= $value['debit_note_id'];
        }
