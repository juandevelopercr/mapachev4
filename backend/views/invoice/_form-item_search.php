...
/* @var $model \backend\models\business\ItemInvoiceForm */
            ...
            <div class="box-body">
                <div id="panel-formulario" style="display: <?= ($itemCount < $maxNumItem) ? 'block' : 'none' ?>">
                    ...
                    $form = ActiveForm::begin([
                        ...
                        'action' => Url::to(["/item-invoice/create_ajax"], GlobalFunctions::URLTYPE)
                    ]); ?>
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            ...
                            $form->field($model, "product_service")->widget(Select2::classname(), [
                                "data" => ItemInvoiceForm::getSelectMap(true),
                        ...
                        <div class="col-sm-3">
                            <?= $form->field($model, 'price_type')->widget(DepDrop::classname(), [
                                ...
                                'pluginOptions' => [
                                    'depends' => ['iteminvoiceform-product_service'],
                    ...
                    <div class="alert alert-info">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div style="margin-right:15px; float: left;">
                                    <a href="<?= Url::to(['/invoice/clone', 'id' => $invoice->id, 'noItem' => 1], GlobalFunctions::URLTYPE) ?>" class="btn btn-primary active">
                                        <i class="glyphicon glyphicon-glyphicon-repeat" aria-hidden="true"></i> Crear Nueva
                                    </a>
                                </div>

                                <div style="margin-right:15px; float: left;">
                                    <a href="<?= Url::to(['/invoice/index'], GlobalFunctions::URLTYPE) ?>" class="btn btn-danger">
...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
	...
	$.ajax({
		...
		success: function(response) {
				$("#iteminvoiceform-quantity").val(1);
				$("#iteminvoiceform-quantity-disp").val(1);
				$("#iteminvoiceform-product_service").val('').trigger("change");
                $("#iteminvoiceform-price_type").val('').trigger("change");
				$("#iteminvoiceform-unit_type_id").val('').trigger("change");
...
$("#iteminvoiceform-product_service").change(function() {
    ...
    if(id_value != '' && id_value != null)
    {
        if(split[0] == 'P')
        {
          ...
          $("#iteminvoiceform-price_type").val("$default_price_type").trigger("change");
        }
        else 
        {
        ...
        $.ajax({
            ...
            success : function(response) 
            {
                $("#iteminvoiceform-product_code").val(response.code);
                $("#iteminvoiceform-unit_type_id").val(response.unit_type_id).trigger("change");
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) 
            {
                $.notify({
    ...
    {
        $("#iteminvoiceform-product_code").val('');
    }
});
