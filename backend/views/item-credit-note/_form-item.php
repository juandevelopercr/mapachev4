...
/* @var $model \backend\models\business\ItemCreditNote */
		...
		<div class="box box-primary">
			...
			<div class="box-body">
		        ...
		        'options' => ['enctype' => 'multipart/form-data'], 'id' => $model->formName(),
		                      'action' => Url::to(["/item-credit-note/update_ajax?id=".$model->id.""], GlobalFunctions::URLTYPE)
		        ]);?>
            	<?= Html::hiddenInput('credit_note_id', $model->credit_note_id); ?>
                <div class="row">
                    <div class="col-md-2">
                ...
                <div class="form-group pull-right">
                    <?= Html::a('<i class="glyphicon glyphicon-remove text-danger"></i> '.Yii::t('backend','Cancelar'), ['index'], ['data-pjax'=>0, 'class' => 'btn btn-default', 'id'=>'btn-cancel-item_credit_note']); ?>&nbsp;
...
$(document).ready(function(e) {
    ...
    $("#itemcreditnote-quantity-disp").keyup(function (e) {
		refresh_prices();
	});	
	
    $("#itemcreditnote-price_unit-disp").keyup(function (e) {
        refresh_prices();
    });	
    
    $("#itemcreditnote-price_type").change(function() 
    {
        ...
        if(id_value != '' && id_value != null)
        {
            ...
            if(url_price_type != '' && url_price_type != null)
            {
                $.ajax({
                    ...
                    {
                        $("#itemcreditnote-price_unit-disp").val(response.price);
                        $("#itemcreditnote-price_unit").val(response.price);
                        refresh_prices();
                    },
    ...
    $("#itemcreditnote-unit_type_id").change(function() 
    {
       refresh_prices();
    });
    
    function refresh_prices() {
        price = $("#itemcreditnote-price_unit-disp").val().replace(/\./g,"");
        price = price.replace(",",".");
        price = parseFloat(price);
        
        quantity = $("#itemcreditnote-quantity-disp").val().replace(/\./g,"");
        ...
        var unit_code_selected = document.getElementById("select2-itemcreditnote-unit_type_id-container").title;
        ...
        $("#itemcreditnote-subtotal-disp").val(new_amount);
        $("#itemcreditnote-subtotal").val(new_amount);
    }
    ...
    $('form#{$model->formName()}').on('beforeSubmit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                    $.pjax.reload({container: '#grid_creditnote-pjax', timeout: 2000});
                    $.notify({
    ...
    $("#btn-cancel-item_credit_note").click(function(e) {
        e.preventDefault();
        showPanels();
    });			
    
    function showPanels() {
        $("#panel-form-item_creditnote").html('');
        $("#panel-grid-item_creditnote").show(15);	
        $.pjax.reload({container: '#grid_creditnote-pjax', timeout: 2000});			
    }
    });
JS;
