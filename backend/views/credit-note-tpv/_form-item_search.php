...
/* @var $model \backend\models\business\ItemCreditNoteForm */
		...
		<div class="box box-primary">
			...
			<div class="box-body">
		        ...
		        'options' => ['enctype' => 'multipart/form-data'], 'id' => $model->formName(),
		                      'action' => Url::to(["/item-credit-note/create_ajax"], GlobalFunctions::URLTYPE)
		        ]);?>
            	<?= $form->field($model, "credit_note_id")->hiddenInput()->label(false) ?>
                <div class="row">
                    <div class="col-sm-3">
                        <?=
                        $form->field($model, "product_service")->widget(Select2::classname(), [
                            "data" => ItemCreditNoteForm::getSelectMap(false),
                    ...
                    <div class="col-sm-3">
                        ...
                        <?= $form->field($model, 'price_type')->widget(DepDrop::classname(), [
                            ...
                            'pluginOptions'=>[
                                'depends'=>['itemcreditnoteform-product_service'],
...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
	...
	$.ajax({
		...
		success: function(response) {
				$("#itemcreditnoteform-product_code").val('');
				$("#itemcreditnoteform-product_code").focus();
				$("#itemcreditnoteform-quantity").val(1);
				$("#itemcreditnoteform-quantity-disp").val(1);
				$("#itemcreditnoteform-product_service").val('').trigger("change");
                $("#itemcreditnoteform-price_type").val('').trigger("change");
				$("#itemcreditnoteform-unit_type_id").val('').trigger("change");
...
$("#itemcreditnoteform-product_service").change(function() {
    ...
    if(id_value != '' && id_value != null)
    {
        if(split[0] == 'P')
        {
          ...
          $("#itemcreditnoteform-price_type").val("$default_price_type").trigger("change");
        }
        else 
        {
        ...
        $.ajax({
            ...
            success : function(response) 
            {
                $("#itemcreditnoteform-product_code").val(response.code);
                $("#itemcreditnoteform-unit_type_id").val(response.unit_type_id).trigger("change");
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) 
            {
                $.notify({
