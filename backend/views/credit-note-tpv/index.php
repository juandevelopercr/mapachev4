...
/* @var $searchModel backend\models\business\CreditNoteSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$controllerId = '/'.$this->context->uniqueId.'/';
$this->title = Yii::t('backend', 'Notas de crédito');
...
<?php 
	...
	$custom_buttons = [
        'update' => function($url, $model) {
            ...
            {
                return Html::a('<i class="glyphicon glyphicon-pencil"></i>', Url::to(['/credit-note-tpv/update', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        },
...
//            {
//                return Html::a('<i class="fa fa-remove text-danger"></i>', ['/credit-note-tpv/correction', 'id' => $model->id], $options);
//            }
//        },
...
//                'target' => '_blank'
//            ];
//            return Html::a('<i class="fa fa-file-text-o"></i>', ['/credit-note-tpv/viewpdfcontingencia', 'id' => $model->id], $options);
//        },
        'pdf' => function($url, $model) {
            $options = [
                'title' => Yii::t('backend', 'NC-PDF'),
                'class' => 'btn btn-xs btn-default btn-flat',
                'aria-label' => Yii::t('backend', 'NC-PDF'),
                'data-method' => 'post',
                'data-pjax' => '0',
                'data-toggle' => 'tooltip',
                'target' => '_blank'
            ];
            return Html::a('<i class="fa fa-file-pdf-o"></i>', Url::to(['/credit-note-tpv/viewpdf', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        'print' => function($url, $model) {
            $options = [
                'title' => Yii::t('backend', 'NC-IMP'),
                'class' => 'btn btn-xs btn-default btn-flat',
                'aria-label' => Yii::t('backend', 'NC-IMP'),
                'data-method' => 'post',
                'data-pjax' => '0',
                'data-toggle' => 'tooltip',
                'target' => '_blank'
            ];
            return Html::a('<i class="fa fa-print"></i>', Url::to(['/credit-note-tpv/printpdf', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
...
//                'target' => '_blank'
//            ];
//            return Html::a('<i class="fa fa-ticket"></i>', ['/credit-note-tpv/viewticketpdf', 'id' => $model->id], $options);
//        },
        'xml' => function($url, $model) {
            $options = [
                'title' => Yii::t('backend', 'NC-XML'),
                'class' => 'btn btn-xs btn-default btn-flat',
                'aria-label' => Yii::t('backend', 'NC-XML'),
                'data-method' => 'post',
                'data-pjax' => '0',
                'data-toggle' => 'tooltip',
                'target' => '_blank'
            ];
            return Html::a('<i class="fa fa-file-code-o"></i>', Url::to(['/credit-note-tpv/viewxml', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        'xmlh' => function($url, $model) {
            $options = [
                'title' => Yii::t('backend', 'NC-MH-XML'),
                'class' => 'btn btn-xs btn-default btn-flat',
                'aria-label' => Yii::t('backend', 'NC-MH-XML'),
                'data-method' => 'post',
                'data-pjax' => '0',
                'data-toggle' => 'tooltip',
                'target' => '_blank'
            ...
            $url_xml_hacienda_verificar = Yii::getAlias('@backend/web/uploads/xmlh/NC-MH-'.$model->key.'.xml');
            $url = Yii::getAlias('/backend/web/uploads/xmlh/NC-MH-'.$model->key.'.xml');
        ...
        'delete' => function($url, $model) {
            ...
            if ($model->status_hacienda === UtilsConstants::HACIENDA_STATUS_NOT_SENT)
            {
                return Html::a('<i class="glyphicon glyphicon-trash"></i>', Url::to(['/credit-note-tpv/delete', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        },
    ];
        ...
        'responsiveWrap' => false,
            ...
            'columns' => [
				...
				[
					...
					'format'=> 'html',
					'value' => function ($data) {
						return Html::a($data->consecutive, Url::to(['/credit-note-tpv/update', 'id' => $data->id], GlobalFunctions::URLTYPE));
					}
				],
				...
				[
					...
					'value'=> function($model){
                        ...
                        return Html::a($temp_name,Url::to(['/customer/view','id'=>$model->customer_id], GlobalFunctions::URLTYPE),['target'=>'_blank','data-pjax'=>0]);
                    },
                ...
                [
                    ...
                    'value'=> function($model){
                        return SellerHasCreditNote::getSellerStringByCreditNote($model->id);
                    },
                    'filterInputOptions'=>['placeholder'=> '------'],
                    'hAlign'=>'center',
                ], 
...
<?php
    ...
    $js = Footer_Bulk_Delete::getFooterBulkDelete($url,'grid_creditnote');
    ...
    $(document).ready(function()
    {
        function init_click_handlers(){
                $("a.btn-send-factura").click(function(e) {
                    e.preventDefault();
                    var selectedId = $("#grid_creditnote").yiiGridView("getSelectedRows");
                ...
                $("a.btn-sendemail-factura").click(function(e) {
                    ...
                    else
                    {
                        ...
                        var url = "'.Url::to(['/credit-note-tpv/enviar-factura-email'], GlobalFunctions::URLTYPE).'?id="+selectedId[0];
                ...
                function cron() {
                    ...
                    if (ids.length > 0)
                    {
                        var url = "'.Url::to(['get-estado-factura-automatico'], GlobalFunctions::URLTYPE).'";						
                        $.ajax({
                            type: \'GET\',
                            url : url,
                            data : {ids:ids},
                            success : function(response) {	
                                if (response.datos.length > 0)		
                                {
                                    ...
                                    lista.forEach( function(valor, indice, array) {
                                        var $mivariable = $(".kv-grid-table tbody tr").filter("[data-key=\"" + valor["id"] + "\"]");
                                        $($mivariable).find(".row-pendiente").each(function(e){
                                            var td = $(this).parent();
                                            $(td).html("");
                                            $(td).html(valor["html"]); 
                                        });
                                        
                                        $($mivariable).find(".col-botones").html(valor["htmlbotones"]);
                                    });
                                    
                                    $.notify({
                                        "message": "Se ha actualizado el estado de algunas facturas.",
                                        "icon": "glyphicon glyphicon-ok-sign",
                                        "title": "Información",							
                                        "showProgressbar": false,
                                        "url":"",						
                                        "target":"_blank"},{"type": "info"}
                                    );								
                                }
                            },
                            error: function(XMLHttpRequest, textStatus, errorThrown) {
                ...
                setInterval(cron, 30000);  // 60000 es un minuto	
                ...
                $("a.btn-imp-tickets").click(function(e) {
                    ...
                    if(selectedId.length == 0) {
                        ...
                        var url = "'.Url::to(['/credit-note-tpv/imp-tickets'], GlobalFunctions::URLTYPE).'?ids="+selectedId;
                        window.location.href= url;
                    }
                });	
        }
