...
<?php 
	...
	$custom_buttons = [
        'update' => function($url, $model) {
            ...
            {
                return Html::a('<i class="glyphicon glyphicon-pencil"></i>', Url::to(['/invoice-tpv/update', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        },
        'corregir' => function($url, $model) {
            ...
            {
                return Html::a('<i class="fa fa-remove text-danger"></i>', Url::to(['/invoice-tpv/correction', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        },
        'pdf_contingencia' => function($url, $model) {
            ...
            return Html::a('<i class="fa fa-file-text-o"></i>', Url::to(['/invoice-tpv/viewpdfcontingencia', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        'pdf' => function($url, $model) {
            ...
            return Html::a('<i class="fa fa-file-pdf-o"></i>', Url::to(['/invoice-tpv/viewpdf', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        'print' => function($url, $model) {
            ...
            if($doc_type === UtilsConstants::PRE_INVOICE_TYPE_INVOICE)
            {
                ...
                return Html::a('<i class="fa fa-print"></i>', Url::to(['/invoice-tpv/printpdf', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        ...
        'ticket' => function($url, $model) {
            ...
            return Html::a('<i class="fa fa-ticket"></i>', Url::to(['/invoice-tpv/viewticketpdf', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        'xml' => function($url, $model) {
            ...
            return Html::a('<i class="fa fa-file-code-o"></i>', Url::to(['/invoice-tpv/viewxml', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
        },
        ...
        'delete' => function($url, $model) {
            ...
            if ($model->status_hacienda === UtilsConstants::HACIENDA_STATUS_NOT_SENT)
            {
                return Html::a('<i class="glyphicon glyphicon-trash"></i>', Url::to(['/invoice-tpv/delete', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
            }
        },
    ];
        ...
        'responsiveWrap' => false,
            ...
            'columns' => [
				...
				[
					...
					'format'=> 'html',
					'value' => function ($data) {
						return Html::a($data->consecutive, ['/invoice-tpv/update', 'id' => $data->id]);
					}
				],
...
<?php
    ...
    $js = Footer_Bulk_Delete::getFooterBulkDelete($url,'grid_invoice_tpv');
...
$this->registerJs('
    $(document).ready(function()
    {
        function init_click_handlers(){
                $("a.btn-send-factura").click(function(e) {
                    e.preventDefault();
                    var selectedId = $("#grid_invoice_tpv").yiiGridView("getSelectedRows");
                    if(selectedId.length == 0) {
                        ...
                        $.ajax({
                            ...
                            success : function(response) {	
                                ...
                                $.pjax.reload({container:"#grid_invoice_tpv-pjax"});			
                                $.notify({
                ...
                $("a.btn-sendemail-factura").click(function(e) {
                    ...
                    else
                    {
                        ...
                        var url = "'.Url::to(['/invoice-tpv/enviar-factura-email'], GlobalFunctions::URLTYPE).'?id="+selectedId[0];
                        //window.location.href= url;
                        $.LoadingOverlay("show");											
                        $.ajax({
                            type: \'GET\',
                ...
                $("a.btn-imp-tickets").click(function(e) {
                    ...
                    if(selectedId.length == 0) {
                        ...
                        var url = "'.Url::to(['/invoice-tpv/imp-tickets'], GlobalFunctions::URLTYPE).'?ids="+selectedId;
                        window.location.href= url;
                    }
                });	
        }
    ...
    $("#grid_invoice_tpv-pjax").on("pjax:success", function() {
