...
if ($model->isNewRecord) {
    $label_update = Yii::t('backend', 'Para adicionar items, debe crear antes la factura');
...
?>
    ...
    <div class="item-invoice-index" style="margin-top:15px;" id="panel-grid-item_invoice">
        <?php echo $this->render('_form-item_search', ['model' => new ItemInvoiceForm(['quantity' => 1, 'invoice_id' => $model->id]), 'customer_id' => $model->customer_id, 'price_assigned'=>$model->customer->price_assigned, 'invoice'=>$model, 'itemCount'=>$model->getItemCount()]); ?>
            ...
            [
                ...
                'buttons' => [
                    ...
                    'update' => function($url, $model) {
                        ...
                        return Html::a('<i class="glyphicon glyphicon-pencil"></i>', Url::to(['/item-invoice/update_ajax','id'=>$model->id], GlobalFunctions::URLTYPE), $options);
                    },
                    'delete' => function ($url, $model) {
                        ...
                        return Html::a('<i class="glyphicon glyphicon-trash"></i>', Url::to(['/item-invoice/delete_ajax', 'id' => $model->id], GlobalFunctions::URLTYPE), $options);
                    }
                ],
            ]
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        $dynagrid = DynaGrid::begin([
                            ...
                            'options' => ['id' => 'grid-item_invoice'] // a unique identifier is important
                        ]);
...
$(document).ready(function(e) {
						 ...
						 if (result)
						 {
							$.ajax({
								...
								success : function(response) {
									...
									$.pjax.reload({container:"#grid-item_invoice-pjax"});
									$.notify({
        ...
        function updateValores()
		{
			...
			if(fID != "")
			{
                $.ajax({
                    type: \'POST\',			
                    dataType: "json",
                    url : "' . Url::to(['/invoice/get-resume-invoice?id=' . $model->id], GlobalFunctions::URLTYPE) . '",
                    data : {id: fID},
	...
	appendClickImage();
	$("#grid-item_invoice-pjax").on("pjax:success", function() {
