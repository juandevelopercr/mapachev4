...
<div class="box box-warning box-solid">
    <div class="box-header with-border">
        <h3 class="box-title"><?= Yii::t('backend','Orden de pedido') ?></h3>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                $form->field($model, "customer_id")->widget(Select2::classname(), [
                    ...
                    "data" =>  (GlobalFunctions::getRol() === User::ROLE_FACTURADOR) ? Customer::getSelectMapByVendedor() : Customer::getSelectMap(),
            ...
            <div class="col-md-3">
                <?= $form->field($model, "commercial_name")->textInput(['maxlength' => true, 'readonly'=>true]) ?>
            </div>            
            ...
            */
            ...
            <div class="col-md-3">
                <?= $form->field($model, 'box_id')->widget(DepDrop::classname(), [
                    ...
                    'pluginOptions' => [
                        'depends' => ['purchaseorder-branch_office_id'],
...
$(document).ready(function(e) {
  ...
  $("#purchaseorder-condition_sale_id").change(function() {
	setcontrols();
  });
  
  $("#purchaseorder-customer_id").change(function() 
  {
    ...
    if(id_value != '' && id_value != null)
    {
        $.ajax({
            ...
            success : function(response) 
            {
                ...
                var collector_id = parseInt(response.collector_id);
                var route_transport_id = parseInt(response.route_transport_id);
                var commercial_name = response.commercial_name;
                
                $("#purchaseorder-condition_sale_id").val(condition_sale_id).trigger("change");	
                
                if(credit_days_id !== 0)
                {
                    $("#purchaseorder-credit_days_id").val(credit_days_id).trigger("change");	
                }
                else 
                {
                    $("#purchaseorder-credit_days_id").val("").trigger("change");	
                }
                
                if(collector_id !== 0)
                {
                    $("#purchaseorder-collector_id").val(collector_id).trigger("change");	
                }
                else 
                {
                    $("#purchaseorder-collector_id").val("").trigger("change");	
                }
                
                if(route_transport_id !== 0)
                {
                    $("#purchaseorder-route_transport_id").val(route_transport_id).trigger("change");	
                }
                else 
                {
                    $("#purchaseorder-route_transport_id").val("").trigger("change");	
                }

                $("#purchaseorder-commercial_name").val(commercial_name);	
            },
  ...
  function setcontrols()  {		
 	 if ($("#purchaseorder-condition_sale_id").val() == "$id_credit")
     {
		$("#purchaseorder-credit_days_id").attr('disabled', false);
     }
	 else
	 {		
		$("#purchaseorder-credit_days_id").attr('disabled', true);			
	 }
  }
