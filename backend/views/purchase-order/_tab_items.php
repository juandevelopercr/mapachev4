...
if ($model->isNewRecord) {
    $label_update = Yii::t('backend', 'Para adicionar items, debe crear antes la orden de pedido');
...
?>
    ...
    <div class="item-purchaseorder-index" style="margin-top:15px;" id="panel-grid-item_purchaseorder">
        <?php echo $this->render('_form-item_search', ['model' => new ItemPurchaseOrderForm(['quantity' => 1, 'purchase_order_id' => $model->id]), 'invoice'=>$model, 'itemCount'=>$model->getItemCount()]); ?>
        ...
        $toolbars = [
            ...
            [
                'content' =>
                Html::a('<i class="fa fa-refresh"></i> ' . Yii::t('backend', 'Resetear'), ['index'], ['class' => 'btn btn-default btn-flat btn-refresh_item_purchaseorder', 'title' => Yii::t('backend', 'Resetear listado'), 'data-toggle' => 'tooltip', 'data-pjax' => 1]),
            ],
            //'{export}',
            '{toggleData}',
        ];
        ...
        $panels = [
            ...
            '<span style="margin-right: 5px;">' .
                Html::a(
                    ...
                    ['#'],
                    ['class' => 'btn btn-delete_item_purchaseorder btn-default']
                ) . '</span>',
        ...
        $columns = [
            ...
            [
                'attribute' => 'exonerate_amount',
                'label' => Yii::t('backend', 'Monto Impuesto exonerado'),
                'contentOptions' => ['class' => 'kv-align-right kv-align-middle'],
                'vAlign' => 'middle',
                'hAlign' => 'center',
                'pageSummary' => true,
                ...
                'value' => function ($data) {
                    return GlobalFunctions::formatNumber($data->exonerate_amount, 2);
                },
                'format' => 'html',
            ],
            ...
            [
                ...
                'value' => function ($data) {
                    return GlobalFunctions::formatNumber($data->getMontoTotalLinea(), 2);
                },
                'format' => 'html',
            ],
                ...
                'buttons' => [
                    ...
                    'update' => function ($url, $model) {
                        $options = [
                            ...
                            'class' => 'btn btn-xs btn-default btn-flat btn-update_item_purchaseorder',
                            'aria-label' => Yii::t('backend', 'Actualizar'),
                            'data-pjax' => '0',
                            'data-toggle' => 'tooltip',
                        ];
                        return Html::a('<i class="glyphicon glyphicon-pencil"></i>', ['/item-purchase-order/update_ajax', 'id' => $model->id], $options);
                    },
                    'delete' => function ($url, $model) {
                        $options = [
                            'title' => Yii::t('backend', 'Eliminar'),
                            'class' => 'btn btn-xs btn-danger btn-flat btn-simple_delete_item_purchaseorder',
                            'aria-label' => Yii::t('backend', 'Eliminar'),
                            'data-pjax' => '0',
                            'data-toggle' => 'tooltip',
                        ];
                        return Html::a('<i class="glyphicon glyphicon-trash"></i>', ['/item-purchase-order/delete_ajax', 'id' => $model->id], $options);
                    }
                ],
            ]
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        $dynagrid = DynaGrid::begin([
                            ...
                            'options' => ['id' => 'grid-item_purchaseorder'] // a unique identifier is important
                        ]);
...
<div id="panel-form-item_purchaseorder">

</div>
<input type="hidden" id="purchaseorder_id" value="<?= $model->id ?>" />

<?php
// Register action buttons js
$this->registerJs('
$(document).ready(function(e) {
	function init_click_handlers_item_purchaseorder(){
	
	$("a.btn-update_item_purchaseorder").click(function(e) {
			e.preventDefault();
				$.ajax({
					type: "GET",
					url : $(this).attr("href"),
					success : function(data) {
						$("#panel-grid-item_purchaseorder").hide(500);
						$("#panel-form-item_purchaseorder").html(data);
					},
        ...
        $("a.btn-simple_delete_item_purchaseorder").click(function(e) {
			e.preventDefault();
						 ...
						 if (result)
						 {
							$.ajax({
								...
								success : function(response) {
									...
									$.pjax.reload({container:"#grid-item_purchaseorder-pjax"});
									$.notify({
        ...
        $("a.btn-delete_item_purchaseorder").click(function(e) {
			e.preventDefault();
            var selectedId = $("#itempurchaseorder").yiiGridView("getSelectedRows");

            if(selectedId.length == 0) {
				bootbox.alert("Seleccione al menos un elemento"); 
            } else {
				var url = "' . Url::to(['/item-purchase-order/deletemultiple_ajax'], GlobalFunctions::URLTYPE) . '";				
				bootbox.confirm({
					message: "¿Est&aacute; seguro que desea eliminar este registro?",
					buttons: {
						confirm: {
		...
		$("a.btn-refresh_item_purchaseorder").click(function(e) {
			e.preventDefault();
            $.pjax.reload({container:"#grid-item_purchaseorder-pjax"});
        });
        ...
        function updateValores()
		{
			...
			if(fID != "")
			{
                $.ajax({
                    type: \'POST\',			
                    dataType: "json",
                    url : "' . Url::to(['/purchase-order/get-resume-purchase-order?id=' . $model->id], GlobalFunctions::URLTYPE) . '",
                    data : {id: fID},
	...
	init_click_handlers_item_purchaseorder(); //first run
	appendClickImage();
	$("#grid-item_purchaseorder-pjax").on("pjax:success", function() {
	    init_click_handlers_item_purchaseorder(); //reactivate links in grid after pjax update
	    appendClickImage();
	});
});
');
