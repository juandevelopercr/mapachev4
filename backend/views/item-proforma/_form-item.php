...
/* @var $model \backend\models\business\ItemProformaForm */
		...
		<div class="box box-primary">
			...
			<div class="box-body">
		        ...
		        'options' => ['enctype' => 'multipart/form-data'], 'id' => $model->formName(),
		                      'action' => $model->isNewRecord ? Url::to(['/item-proforma/create_ajax'], GlobalFunctions::URLTYPE): Url::to(["/item-proforma/update_ajax?id=".$model->id.""], GlobalFunctions::URLTYPE)
		        ]);?>
            	<?= Html::hiddenInput('proforma_id', $model->proforma_id); ?>
                <div class="row">
                    <div class="col-md-2">
                ...
                <div class="form-group pull-right">
                    <?= Html::a('<i class="glyphicon glyphicon-remove text-danger"></i> '.Yii::t('backend','Cancelar'), ['index'], ['data-pjax'=>0, 'class' => 'btn btn-default', 'id'=>'btn-cancel-item_proforma']); ?>&nbsp;
...
$(document).ready(function(e) {
    ...
    $("#itemproforma-quantity-disp").keyup(function (e) {
		refresh_prices();
	});	
	
    $("#itemproforma-price_unit-disp").keyup(function (e) {
        refresh_prices();
    });	
    
    $("#itemproforma-price_type").change(function() 
    {
        var id_value = $(this).val();
        if(id_value != '' && id_value != null)
        {
            var url_price_type = "$url_get_price_by_type";
            var unit_type = $("#itemproforma-unit_type_id").val();
            if(url_price_type != '' && url_price_type != null)
            {
                $.ajax({
                    ...
                    {
                        $("#itemproforma-price_unit-disp").val(response.price);
                        $("#itemproforma-price_unit").val(response.price);
                        refresh_prices();
                    },
    ...
    $("#itemproforma-unit_type_id").change(function() 
    {
        var id_value = $("#itemproforma-price_type").val();
        if(id_value != '' && id_value != null)
        {
            var url_price_type = "$url_get_price_by_type";
            var unit_type = $("#itemproforma-unit_type_id").val();
    ...
    */
    ...
    function refresh_prices() {
        price = $("#itemproforma-price_unit-disp").val().replace(/\./g,"");
        price = price.replace(",",".");
        price = parseFloat(price);
        
        quantity = $("#itemproforma-quantity-disp").val().replace(/\./g,"");
        ...
        var unit_code_selected = document.getElementById("select2-itemproforma-unit_type_id-container").title;
        ...
        $("#itemproforma-subtotal-disp").val(new_amount);
        $("#itemproforma-subtotal").val(new_amount);
    }
    ...
    $('form#{$model->formName()}').on('beforeSubmit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                    $.pjax.reload({container: '#grid-item_proforma-pjax', timeout: 2000});
                    $.notify({
    ...
    $("#btn-cancel-item_proforma").click(function(e) {
        e.preventDefault();
        showPanels();
    });			
    
    function showPanels() {
        $("#panel-form-item_proforma").html('');
        $("#panel-grid-item_proforma").show(15);	
        $.pjax.reload({container: '#grid-item_proforma-pjax', timeout: 2000});			
    }
    });
JS;
