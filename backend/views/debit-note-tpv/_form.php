...
<div class="box box-default box-solid">
    <div class="box-header with-border">
        <h3 class="box-title"><?= Yii::t('backend','Nota de débito') ?></h3>
                ...
                <fieldset id="pedido_minimo_fieldset" style="width: 100%; border: 1px solid #C0C0C0; margin-top:15px">
                    ...
                    <div class="col-md-6">
                        ...
                        $form->field($model, "reference_code")->widget(Select2::classname(), [
                            "data" => ReferenceCode::getSelectMap(),
                            "language" => Yii::$app->language,
                            "pluginOptions" => [
                                "allowClear" => true
                            ],
...
$(document).ready(function(e) {
  ...
  $("#debitnote-condition_sale_id").change(function() {
	setcontrols();
  });
  
  $("#debitnote-customer_id").change(function() 
  {
    ...
    if(id_value != '' && id_value != null)
    {
        $.ajax({
            ...
            success : function(response) 
            {
                ...
                var seller_id = parseInt(response.seller_id);
                var invoice_type = parseInt(response.invoice_type);
                
                $("#debitnote-condition_sale_id").val(condition_sale_id).trigger("change");	
                
                if(credit_days_id !== 0)
                {
                    $("#debitnote-credit_days_id").val(credit_days_id).trigger("change");	
                }
                else 
                {
                    $("#debitnote-credit_days_id").val("").trigger("change");	
                }
                
                if(seller_id !== 0)
                {
                    $("#debitnote-seller_id").val(seller_id).trigger("change");	
                }
                else 
                {
                    $("#debitnote-seller_id").val("").trigger("change");	
                }
                $("#debitnote-invoice_type").val(invoice_type).trigger("change");
            },
  ...
  function setcontrols()  {		
 	 if ($("#debitnote-condition_sale_id").val() == "$id_credit")
     {
		$("#debitnote-credit_days_id").attr('disabled', false);
     }
	 else
	 {		
		$("#debitnote-credit_days_id").attr('disabled', true);			
	 }
  }
