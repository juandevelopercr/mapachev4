            ...
            <div class="box-body">
                <div id="panel-formulario" style="display: <?= ($itemCount < $maxNumItem) ? 'block' : 'none' ?>">
                    ...
                    $form = ActiveForm::begin([
                        ...
                        'action' => Url::to(["/item-proforma/create_ajax"], GlobalFunctions::URLTYPE)
                    ]); ?>
                    <?= $form->field($model, "proforma_id")->hiddenInput()->label(false) ?>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            ...
                            $form->field($model, "product_service")->widget(Select2::classname(), [
                                "data" => ItemProformaForm::getSelectMap(true),
                        ...
                        <div class="col-sm-2">
                            <?= $form->field($model, 'price_type')->widget(DepDrop::classname(), [
                                ...
                                'pluginOptions' => [
                                    'depends' => ['itemproformaform-product_service'],
                ...
                <div id="panel-informacion" style="display: <?= ($itemCount >= $maxNumItem) ? 'block' : 'none' ?>; width: 100%;">
                    <br />
                    <div class="alert alert-info">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div style="margin-right:15px; float: left;">
                                    <a href="<?= Url::to(['/proforma/clone', 'id' => $invoice->id, 'noItem' => 1], GlobalFunctions::URLTYPE) ?>" class="btn btn-primary active">
                                        <i class="glyphicon glyphicon-glyphicon-repeat" aria-hidden="true"></i> Crear Nueva
                                    </a>
                                </div>

                                <div style="margin-right:15px; float: left;">
                                    <a href="<?= Url::to(['/proforma/index'], GlobalFunctions::URLTYPE) ?>" class="btn btn-danger">
...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
	...
	$.ajax({
		...
		success: function(response) {
				$("#itemproformaform-quantity").val(1);
				$("#itemproformaform-quantity-disp").val(1);
				$("#itemproformaform-product_service").val('').trigger("change");
                $("#itemproformaform-price_type").val('').trigger("change");
				$("#itemproformaform-unit_type_id").val('').trigger("change");
...
$("#itemproformaform-product_service").change(function() {
    ...
    if(id_value != '' && id_value != null)
    {
        if(split[0] == 'P')
        {
          ...
          $("#itemproformaform-price_type").val("$default_price_type").trigger("change");
        }
        else 
        {
        ...
        $.ajax({
            ...
            success : function(response) 
            {
                 $("#itemproformaform-product_code").val(response.code);
                $("#itemproformaform-unit_type_id").val(response.unit_type_id).trigger("change");
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) 
            {
                $.notify({
    ...
    {
        $("#itemproformaform-product_code").val('');
    }
});
