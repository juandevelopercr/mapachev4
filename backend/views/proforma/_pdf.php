...
/* @var $proforma backend\models\business\Proforma */
/* @var $items_proforma backend\models\business\ItemProforma[] */
            ...
            <table width="100%">
                <tr>
                    <td style="background-color: royalblue; color: white; padding: 5px;">
                        <h5>Proforma: <?= $proforma->consecutive ?></h5>
                    </td>
                </tr>
...
<table width="100%">
    <tr>
        <td width="45%"  valign="top" style="background-color: #e5e7ea; padding: 5px;">
            <span style="font-size: 13px;"><strong><?= $proforma->customer->name ?></strong></span><br />
            <span style="font-size: 12px;">Identificación: <?= $proforma->customer->identification ?></span><br />
            <span style="font-size: 12px;">Dirección: <?= wordwrap( $proforma->customer->address, 50 ) ?></span><br />
            <span style="font-size: 12px;">Teléfono: <?= $proforma->customer->phone ?></span><br />
            <span style="font-size: 12px;">Correo: <?= $proforma->customer->email ?></span><br />
            <span style="font-size: 12px;">Condición de venta: <?= $proforma->conditionSale->name ?></span><br />
            <?php
            if($proforma->condition_sale_id == ConditionSale::getIdCreditConditionSale())
            {
                echo '<span style="font-size: 12px;">Plazo crédito:'.$proforma->creditDays->name.'</span><br />';
            }
            ?>
        </td>
        <td width="5%">
        ...
        <td width="45%" align="right" valign="top" style="background-color: #e5e7ea; padding: 5px;">
            ...
            <span style="font-size: 12px;"><strong><?= Yii::t('backend','Fecha de solicitud').': </strong>'.GlobalFunctions::formatDateToShowInSystem($proforma->request_date) ?></span><br/>
            <span style="font-size: 12px;"><strong><?= Yii::t('backend','Estado').': </strong>'.UtilsConstants::getProformaStatusSelectType($proforma->status) ?></span><br/>
            <span style="font-size: 12px;"><strong><?= Yii::t('backend','Sucursal').': </strong>'.$proforma->branchOffice->name ?></span><br/>
            <br/>
...
<table class="table-bordered" border="0" cellpadding="5" cellspacing="0" width="100%">
    ...
    foreach ($items_proforma as $index => $item)
    {
        ?>
        <tr>
            <td align="left">
            ...
            <td align="right">
                <span style="font-size: 10px; font-weight: normal;">
                    ...
                    if ($proforma->currency->symbol == 'CRC' && $moneda != 'COLONES')
                        $price_unit = $price_unit / $issuer->change_type_dollar;
                    else
                        if ($proforma->currency->symbol == 'USD' && $moneda != 'DOLAR')
                            $price_unit = $price_unit * $issuer->change_type_dollar;
                    ?>
            ...
            <td align="right">
                <span style="font-size: 10px; font-weight: normal;">
                    ...
                    $discount_percent = 0;
                    if ($item->price_unit > 0 && $item->quantity > 0)
    ...
    $model = Proforma::getResumeProforma($proforma->id);
    ...
    elseif ($proforma->currency->symbol == 'USD' && $moneda != 'DOLAR')
    {
        $total_subtotal = $model->subtotal * $issuer->change_type_dollar;
        $total_tax = $model->tax_amount * $issuer->change_type_dollar;
        $total_discount = $model->discount_amount * $issuer->change_type_dollar;
...
<table width="100%">
    <tr>
        <td width="35%">
            <span style="font-size: 12px;"><strong><?= Yii::t('backend','Tiempos de entrega') ?>: </strong><?= $proforma->delivery_time .' '.UtilsConstants::getDeliveryTimesSelectType($proforma->delivery_time_type) ?></span><br/>
        </td>
        <td width="30%">
        ...
        <td width="35%" align="right" valign="top">
            <span style="font-size: 12px;">** <?= Yii::t('backend','Presupuesto válido por 8 días') ?> **</span><br/>
