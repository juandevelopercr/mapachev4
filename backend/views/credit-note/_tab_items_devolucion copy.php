...
if ($model->isNewRecord) {
    $label_update = Yii::t('backend', 'Para adicionar items, debe crear antes la nota de crédito');
...
?>
    ...
    $panels = [
      ...
      $columns = [
            ...
            [
              ...
              'value' => function ($data) {
                  $quantity_actual = $data->getQuantityNotaActual();   
                  if ($data->quantity > 0)                
                    $quantity_actual = $quantity_actual - ($quantity_actual - $data->quantity);
                  return GlobalFunctions::formatNumber($quantity_actual,0);
              },
              'format' => 'html',
            ],
            [
                ...
                'value' => function ($data) {
                    ...
                    $quantity = 0;
                    if ($data->quantity > 0)                
                      $quantity = $quantity_actual - $data->quantity;
                    return GlobalFunctions::formatNumber($quantity,0);
                },
                'format' => 'html',
              ],
            [
                'attribute'=>'quantity_devuelta',
                'contentOptions'=>['class'=>'kv-align-center kv-align-middle'],
                'vAlign' => 'middle',
                'hAlign' => 'center',
                'pageSummary' => true,
                ...
                'value' => function ($data) {
                    $quantity_actual = $data->getQuantityNotaActual();        
                    $control = "<input type=\"hidden\" name=\"row_id[]\" value=\"".$data->id."\">";
                    $control .= "<input type=\"hidden\" name=\"quantity[]\" value=\"".(int)$quantity_actual."\">";
                    $control .= "<input type=\"number\" name=\"quantity_dev[]\" max=\"".(int)$quantity_actual."\" min=\"0\" size=\"10\" value=\"0\">";
                    return $control;
                },
                'format' => 'raw',
              ],            
    ...
    <?php 
                    $dynagrid = DynaGrid::begin([			                
                                ...
                                'options' => ['id' => 'grid_creditnote'] // a unique identifier is important
                    ]);
...
<div id="panel-form-item_creditnote">
