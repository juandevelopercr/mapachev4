...
class CreditNoteController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if ($model->save()) {
                    ...
                    //SellerHasCreditNote::updateRelation($model, [], 'sellers', 'seller_id');

                    //CollectorHasCreditNote::updateRelation($model, [], 'collectors', 'collector_id');
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($model) && !empty($model)) {
            ...
            /*
            //BEGIN seller has credit_note
            $seller_assigned = SellerHasCreditNote::getSellerByCreditNoteId($id);
            ...
            $model->sellers = $seller_assigned_ids;
            //END seller method has credit_note


            //BEGIN collector has credit_note
            $collector_assigned = CollectorHasCreditNote::getCollectorByCreditNoteId($id);
            ...
            */
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                try {
                    ...
                    //SellerHasCreditNote::updateRelation($model, $seller_assigned, 'sellers', 'seller_id');

                    //CollectorHasCreditNote::updateRelation($model, $collector_assigned, 'collectors', 'collector_id');
    ...
    public function getEstadoFactura($credit_note, $emisor)
    {
        ...
        if ($status_hacienda === UtilsConstants::HACIENDA_STATUS_RECEIVED) {
            ...
            $mensaje = 'Solo se puede consultar el estado de una nota de crédito que haya sido recibida en hacienda';
            $type = 'warning';
            $titulo = "Informaci&oacute;n <hr class=\"kv-alert-separator\">";
            $actualizar = 0;
        }
    ...
    public function actionDevolucion($invoice_id)
    {
        $model = new CreditNote();
        $model->LoadDefaultValues();
        $invoice = Invoice::findOne($invoice_id);
        ...
        $model->reference_number = $invoice->key;
        $model->reference_emission_date = date('Y-m-d H:i:s');
        $model->reference_code = '02'; // corrige monto
        $model->reference_reason = 'Devolución de Mercancia';
        $model->consecutive = $model->generateConsecutive();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if ($model->reference_code == '01') {
                    $model->credit_note_type = UtilsConstants::CREDIT_NOTE_TYPE_TOTAL;
                    // El estado se cambia cuando se acepta la nota
                    //$invoice->status_hacienda = UtilsConstants::HACIENDA_STATUS_CREDIT_NOTE;
                } else {
                    $model->credit_note_type = UtilsConstants::CREDIT_NOTE_TYPE_PARTIAL;
                    //$invoice->status_hacienda = UtilsConstants::HACIENDA_STATUS_CREDIT_NOTE_PARTIAL;
                ...
                $list_quantity_dev = Yii::$app->request->post()['quantity_dev'];     
                $list_quantity = Yii::$app->request->post()['quantity'];     
        
                $validacion = $this->validaDevolucion($ids, $list_quantity_dev, $list_quantity);
                if ($validacion == false)
                {
                    GlobalFunctions::addFlashMessage('danger', Yii::t('backend', 'La cantidad a devolver de algunos items es superior a la cantidad existente. Corrija la información e inténtelo nuevamente'));
                    $transaction->rollBack();
                    return $this->redirect(['devolucion', 'id' => $invoice_id]);
                }
                ...
                if ($model->save()) {
                    ...
                    */
                    ...
                    foreach ($ids as $id){
                        $quantity_dev = $list_quantity_dev[$index];
                        if ($quantity_dev > 0){
                            $itemInvoice = ItemInvoice::find()->where(['id'=>$id])->one();
                            $newItemCreditNote = new ItemCreditNote;
                            $newItemCreditNote->attributes = $itemInvoice->attributes;
                            $newItemCreditNote->quantity = $quantity_dev;
                            $newItemCreditNote->credit_note_id = $model->id;
                            $newItemCreditNote->save();                            
                            $result = UtilsConstants::devolucionItemCreditNote($newItemCreditNote, $quantity_dev);
                        }
                        $index++;
                    }
                    GlobalFunctions::addFlashMessage('success', Yii::t('backend', 'Se ha creado la devolución satisfactoriamente'));
    ...
    public function validaDevolucion($ids, $list_quantity_dev, $list_quantity)
    {
        $result = true;
        $index = 0;
        foreach ($ids as $id){
            if ($list_quantity[$index] < $list_quantity_dev[$index])
            {
                $result = false;
                break;
            }
    ...
    public function actionUpdateDevolucion($id)
    {
        ...
        if (isset($model) && !empty($model)) {
            $searchModelItems = ItemCreditNote::find()->where(['credit_note_id'=>$model->id])->all();
            if (is_null($model->status_hacienda)) {
                $model->status_hacienda = UtilsConstants::HACIENDA_STATUS_NOT_SENT;
            }
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($ready_to_send === 1 && $total_items === 0) {
                    ...
                    return $this->render('updateDevolucion', [
                        'model' => $model,
                        'searchModelItems' => $searchModelItems,
                        'dataProviderItems' => $dataProviderItems,
                    ]);
            ...
            if (isset(Yii::$app->request->post()['btn-devolucion']))
            {
                $ids = Yii::$app->request->post()['row_id'];               
                $list_quantity_dev = Yii::$app->request->post()['quantity_dev'];     
                $list_quantity = Yii::$app->request->post()['quantity'];     
                ...
                $sumQuantity_dev = 0;
                foreach ($ids as $id){
                    $sumQuantity += $list_quantity[$index];
                    $sumQuantity_dev += $list_quantity_dev[$index];
                    $index++;
                }

                //    die(var_dump($sumQuantity. ' - '.$sumQuantity_dev));

                if ($sumQuantity == $sumQuantity_dev){
                    // Se está haciendo una nota de cancelacion
                    GlobalFunctions::addFlashMessage('warning', Yii::t('backend', 'No está devolviendo ningún item, revise e inténtelo de nuevo'));
                }
                else
                if ($sumQuantity_dev == 0){
                    // Se está haciendo una nota de cancelacion
                    GlobalFunctions::addFlashMessage('warning', Yii::t('backend', 'Está tratando de devolver todos los items de la factura. Si desea cancelar la factura realice una nota de crédito para anularla en su totalidad'));
                }
                else
                {
                    $index = 0;
                    foreach ($ids as $id){
                        ...
                        $quantity_dev = $list_quantity_dev[$index];
                        $item = ItemCreditNote::findOne($id);
                        if (!is_null($item))
                            $result = UtilsConstants::devolucionItemCreditNote($id, $quantity_dev, $quantity);
                        $index++;
                    }
                    GlobalFunctions::addFlashMessage('warning', Yii::t('backend', 'Se ha creado la devolución satisfactoriamente'));
                    return $this->redirect(['index']);
                }
            }
        } else {
