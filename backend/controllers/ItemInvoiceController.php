...
/**
 * ItemInvoiceController implements the CRUD actions for ItemInvoice model.
 */
class ItemInvoiceController extends Controller
{
    ...
    public function actionCreate_ajax()
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            $array_posted = Yii::$app->request->post('ItemInvoiceForm');
            ...
            $msg_ajuste_cantidad = '';
            $datainvoice = Invoice::find()->where(['id'=>$array_posted['invoice_id']])->one();
            $pid = $explode[1];
    ...
    public function actionDeletemultiple_ajax()
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($model->delete()) {
                //Actualizar los totales de la factura
                $invoice = Invoice::find()->where(['id' => $tempinvoice_id])->one();
                $invoice->save(false);
                $eliminados++;
            } else {
                $noeliminados++;
