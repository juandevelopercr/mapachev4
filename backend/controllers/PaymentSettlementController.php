...
/**
 * PaymentSettlementController implements the CRUD actions for Invoice model.
 */
class PaymentSettlementController extends Controller
{
    ...
    {        
        ...
        $dataProviderPendientes = $searchModelPendientes->PaymentSettlementSearch(Yii::$app->request->queryParams);
...
/**
    ...
    public function actionGetResumePayment()
    {
        $ids = Yii::$app->request->post()['ids'];
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;

        $datos = InvoiceAbonos::find()->select('sum(amount) as amount, max(payment_method.code) as payment_method')
                                      ...
                                      ->orderBy('payment_method.code')
                                      ->asArray()
                                      ->all();

        $total = Invoice::find()->where(['id'=>$ids])->sum('total_comprobante');
        if (is_null($total))
            $total = 0;
        ...
        foreach ($datos as $d){
            switch ($d['payment_method']) {
                ...
                case '06':
                    # SINPE MOVIL
                    $sinpe_movil = $d['amount'];
                    break;                                                                       
            }
        }
       
        $total_metodos = $efectivo + $tarjeta + $cheque + $transferencia + $recaudo + $sinpe_movil;
        $diferencia = sprintf('%0.2f', $total) - sprintf('%0.2f', $total_metodos); 

        return \Yii::$app->response->data = [
            'efectivo' => GlobalFunctions::formatNumber($efectivo, 2),
            'tarjeta' => GlobalFunctions::formatNumber($tarjeta, 2),
            'cheque' => GlobalFunctions::formatNumber($cheque, 2),
            'transferencia' => GlobalFunctions::formatNumber($transferencia, 2),
            'recaudo' => GlobalFunctions::formatNumber($recaudo, 2),
            'sinpemovil' => GlobalFunctions::formatNumber($sinpe_movil, 2),
            'total' => GlobalFunctions::formatNumber($total, 2),
            'diferencia' => GlobalFunctions::formatNumber($diferencia, 2),
        ];
    }    
