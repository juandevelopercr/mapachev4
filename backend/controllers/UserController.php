...
use backend\models\business\UserHasSeller;
use Yii;
...
class UserController extends Controller
{
    ...
    public function goHomeTpv()
    {
        $this->redirect("/tpv/invoice/index");
        //$url = Yii::$app->getBaseUrl(). '/tpv/invoice/index';
	...
	public function actionCreate()
	{
		...
		{
		    if($model->role == User::ROLE_FACTURADOR && empty($model->branch_office_id))
            {
                $model->addError('branch_office_id',Yii::t('backend','Sucursal no puede estar vacía'));
			...
			if($model->save(true,$allScenarios[$model->scenario]))
			{
				if($model->save())
                {        
                    //$model->sellers = Yii::$app->request->post()['User']['sellers'];         
                    //UserHasSeller::updateRelation($model, [], 'sellers', 'seller_id');
	...
	public function actionUpdate($id)
	{
        ...
        $seller_assigned = UserHasSeller::getVendedorByUserId($id);
        ...
        */
		...
		if($model->load(Yii::$app->request->post()))
		{
			...
			if($model->save(true,$allScenarios[$model->scenario]))
			{
                if($model->save())
                {
                   // UserHasSeller::updateRelation($model, $seller_assigned, 'sellers', 'seller_id');
