...
class ApiXML
{
	...
	function genXMLFe($emisor, $factura, $factura_detalles)
	{
        ...
        if ($factura->invoice_type == UtilsConstants::PRE_INVOICE_TYPE_INVOICE){
            $root = $doc->createElementNS(
                'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/facturaElectronica','FacturaElectronica');
            ...
            $root->setAttributeNS(
                'http://www.w3.org/2001/XMLSchema-instance',
                'xsi:schemaLocation',
                'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/facturaElectronica https://tribunet.hacienda.go.cr/docs/esquemas/2017/v4.3/facturaElectronica.xsd'
            );
        }
        else
        {
            $root = $doc->createElementNS(
                'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/tiqueteElectronico','TiqueteElectronico');
            $doc->appendChild($root);
            $root->setAttributeNS(
                'http://www.w3.org/2000/xmlns/' ,
                'xmlns:xsd',
            ...
            $root->setAttributeNS(
                'http://www.w3.org/2001/XMLSchema-instance',
                'xsi:schemaLocation',
                'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/tiqueteElectronico https://tribunet.hacienda.go.cr/docs/esquemas/2017/v4.3/tiqueteElectronico.xsd'
            );
        }
        ...
        if ($factura->customer->identificationType->code == '05') // Pasaporte entonces poner el código 
        {
            $nodo = $doc->createElement('IdentificacionExtranjero', htmlspecialchars($factura->customer->foreign_identification));
            $receptor->appendChild($nodo);
        }
        else		
        {
		...
		if (!is_null($factura->customer->country_code_fax) && !empty($factura->customer->country_code_fax) && !is_null($factura->customer->fax) && !empty($factura->customer->fax)){
			$fax = $doc->createElement('Fax');
			$receptor->appendChild($fax);
		
			$nodo = $doc->createElement('CodigoPais', $factura->customer->country_code_fax);
			$fax->appendChild($nodo);
		
			$nodo = $doc->createElement('NumTelefono', $factura->customer->fax);
			$fax->appendChild($nodo);
		}
		...
		$nodo = $doc->createElement('TipoCambio', number_format($factura->change_type, 5, '.', ''));
		$codigo->appendChild($nodo);		
     ...
     * @return string
     */
    function genXMLNC($emisor, $credit_note, $credit_note_items)
    {
        $fecha = date('c');
        $plazo_credito = $credit_note->conditionSale->code == '02' ? $credit_note->creditDays->name: '0';

        $doc  = new \DomDocument('1.0','UTF-8');
        $doc->formatOutput = true;

        $root = $doc->createElementNS('https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/notaCreditoElectronica', 'NotaCreditoElectronica');
        $doc->appendChild($root);
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/' ,'xmlns:xsd', 'http://www.w3.org/2001/XMLSchema');
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/' ,'xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance');
        $root->setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:schemaLocation', 'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/notaCreditoElectronica'.' '.'https://tribunet.hacienda.go.cr/docs/esquemas/2017/v4.3/notaCreditoElectronica.xsd');


        $nodo = $doc->createElement('Clave', $credit_note->key);
        $root->appendChild($nodo);

        $nodo = $doc->createElement('CodigoActividad', $emisor->code_economic_activity);
        $root->appendChild($nodo);

        $nodo = $doc->createElement('NumeroConsecutivo', $credit_note->consecutive);
        $root->appendChild($nodo);
        ...
        $nodo = $doc->createElement('Nombre', htmlspecialchars($credit_note->customer->name));
        $receptor->appendChild($nodo);

        if ($credit_note->customer->identificationType->code == '05') // Pasaporte entonces poner el código 
        {
            $nodo = $doc->createElement('IdentificacionExtranjero', htmlspecialchars($credit_note->customer->foreign_identification));
            $receptor->appendChild($nodo);
        }
        else		
        {
            $campo_tipo_identificacion = trim($credit_note->customer->identificationType->code);
            $campo_identificacion = trim($credit_note->customer->identification);
        ...
        if (!is_null($credit_note->customer->commercial_name) && !empty($credit_note->customer->commercial_name))
        {
            $nodo = $doc->createElement('NombreComercial', htmlspecialchars($credit_note->customer->commercial_name));
            $receptor->appendChild($nodo);
        }

        if (!is_null($credit_note->customer->province_id) && !is_null($credit_note->customer->canton_id) && !is_null($credit_note->customer->disctrict_id))
        {
            $ubicacion = $doc->createElement('Ubicacion');
            $receptor->appendChild($ubicacion);

            $nodo = $doc->createElement('Provincia', $credit_note->customer->province->code);
            $ubicacion->appendChild($nodo);

            $nodo = $doc->createElement('Canton', str_pad($credit_note->customer->canton->code, 2, '0', STR_PAD_LEFT));
            $ubicacion->appendChild($nodo);

            $nodo = $doc->createElement('Distrito', str_pad($credit_note->customer->disctrict->code, 2, '0', STR_PAD_LEFT));
            $ubicacion->appendChild($nodo);

            if (!is_null($credit_note->customer->other_signs) && !empty($credit_note->customer->other_signs)){
                $nodo = $doc->createElement('OtrasSenas', htmlspecialchars($credit_note->customer->other_signs));
                $ubicacion->appendChild($nodo);
            }
            else
            {
        ...
        if (!is_null($credit_note->customer->country_code_phone) && !empty($credit_note->customer->country_code_phone) && !is_null($credit_note->customer->phone) && !empty($credit_note->customer->phone)){
            $telefono = $doc->createElement('Telefono');
            $receptor->appendChild($telefono);

            $nodo = $doc->createElement('CodigoPais', $credit_note->customer->country_code_phone);
            $telefono->appendChild($nodo);

            $nodo = $doc->createElement('NumTelefono', $credit_note->customer->phone);
            $telefono->appendChild($nodo);
        }

        if (!is_null($credit_note->customer->country_code_fax) && !empty($credit_note->customer->country_code_fax) && !is_null($credit_note->customer->fax) && !empty($credit_note->customer->fax)){
            $fax = $doc->createElement('Fax');
            $receptor->appendChild($fax);

            $nodo = $doc->createElement('CodigoPais', $credit_note->customer->country_code_fax);
            $fax->appendChild($nodo);

            $nodo = $doc->createElement('NumTelefono', $credit_note->customer->fax);
            $fax->appendChild($nodo);
        }

        if (!is_null($credit_note->customer->email) && !empty($credit_note->customer->email))
        {
            $nodo = $doc->createElement('CorreoElectronico', $credit_note->customer->email);
            $receptor->appendChild($nodo);
        }

        // Otros elementos
        $nodo = $doc->createElement('CondicionVenta', $credit_note->conditionSale->code);
        $root->appendChild($nodo);

        $condition_sale_id = (int) $credit_note->condition_sale_id;

        if ($condition_sale_id === ConditionSale::getIdCreditConditionSale()) // Crédito
        {
            $credit_days = (int) $credit_note->creditDays->name;
            $nodo = $doc->createElement('PlazoCredito', $credit_days);
            $root->appendChild($nodo);
        }

        $paymentMethods = PaymentMethodHasCreditNote::find()->where(['credit_note_id' => $credit_note->id])->all();
        ...
        if ($credit_note->currency_id === Currency::getCurrencyIdByCode('USD'))
        {
            $strmoneda = 'DOLARES';
        }
        else
        ...
        foreach ($credit_note_items as $fdetalle)
        {
            $linea = $doc->createElement('LineaDetalle');
            $detalle->appendChild($linea);
        ...
        $nodo = $doc->createElement('CodigoMoneda', $credit_note->currency->symbol);
        $codigo->appendChild($nodo);

        $nodo = $doc->createElement('TipoCambio', number_format($credit_note->change_type, 5, '.', ''));
        $codigo->appendChild($nodo);

        $nodo = $doc->createElement('TotalServGravados', number_format($credit_note->totalServGravados, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalServExentos', number_format($credit_note->totalServExentos, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalServExonerado', number_format($credit_note->totalMontoServExonerado, 5, '.', ''));
        $resumen->appendChild($nodo);


        $nodo = $doc->createElement('TotalMercanciasGravadas', number_format($credit_note->totalMercanciasGravadas, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalMercanciasExentas', number_format($credit_note->totalMercanciasExentas, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalMercExonerada', number_format($credit_note->totalMontoMercExonerado, 5, '.', ''));
        $resumen->appendChild($nodo);


        $nodo = $doc->createElement('TotalGravado', number_format($credit_note->totalGravado, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalExento', number_format($credit_note->totalExento, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalExonerado', number_format($credit_note->totalExonerado, 5, '.', ''));
        $resumen->appendChild($nodo);


        $nodo = $doc->createElement('TotalVenta', number_format($credit_note->totalVenta, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalDescuentos', number_format($credit_note->totalDescuentos, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalVentaNeta', number_format($credit_note->totalVentaNeta, 5, '.', ''));
        $resumen->appendChild($nodo);


        $nodo = $doc->createElement('TotalImpuesto', number_format($credit_note->totalImpuesto, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalIVADevuelto', number_format(0, 5, '.', ''));
        $resumen->appendChild($nodo);

        $nodo = $doc->createElement('TotalComprobante', number_format($credit_note->total_comprobante, 5, '.', ''));
        $resumen->appendChild($nodo);
        ...
        $nodo = $doc->createElement('Numero', $credit_note->reference_number);
        $referencia->appendChild($nodo);

        $fecha_r = date('Y-m-d', strtotime($credit_note->reference_emission_date));
        $fecha_referencia = date('c', strtotime($fecha_r));
        $nodo = $doc->createElement('FechaEmision', $fecha_referencia);
        $referencia->appendChild($nodo);

        $nodo = $doc->createElement('Codigo', $credit_note->reference_code);
        $referencia->appendChild($nodo);

        $nodo = $doc->createElement('Razon', $credit_note->reference_reason);
        $referencia->appendChild($nodo);
