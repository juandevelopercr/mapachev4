...
class PasswordResetRequest extends Model
{
     ...
     */
    public function sendEmail($is_to_api = false)
    {
        /* @var $user User */
        $class = Yii::$app->getUser()->identityClass ? : 'common\models\User';
        $user = $class::findOne([
        ...
        if ($user)
        {
            ...
            if ($user->save())
            {
                if($is_to_api)
                {
                    $email_template = 'passwordAPIResetToken';
                }
                else
                {
                    $email_template = 'passwordResetToken';
                }
                $message = Yii::$app->mail->compose(['html' => $email_template.'-html', 'text' => $email_template.'-text'], ['user' => $user])
                    ->setTo($user->email)
