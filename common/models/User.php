...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    const ROLE_FACTURADOR = 'Facturador';
	...
	{
		return [
			self::SCENARIO_CREATE => ['username', 'email', 'auth_key', 'auth_key_test', 'name', 'last_name','status','switch_status','password_hash','role','seniority', 'skills', 'personal_stuff','position','created_at','updated_at','branch_office_id', 'box_id'],
			self::SCENARIO_UPDATE => ['username', 'email', 'auth_key', 'auth_key_test', 'name', 'last_name','status','switch_status','password_hash','role','seniority', 'skills', 'personal_stuff','position','created_at','updated_at','branch_office_id', 'box_id'], 
            self::SCENARIO_CHANGE_PASSWORD => [ 'auth_key','password_hash','created_at','updated_at'],
            self::SCENARIO_SING_UP => ['username', 'email', 'auth_key', 'name', 'last_name','password_hash','seniority', 'skills', 'personal_stuff','position','created_at','updated_at'],
            self::SCENARIO_RESET_PASSWORD => ['email', 'password_hash','auth_key','password_reset_token','created_at','updated_at'],
        ];
	}
	...
	public function rules()
	{
		...
		return [
			//required
				[['username', 'email', 'auth_key', 'auth_key_test', 'name', 'last_name','status','switch_status','password_hash','role', 'branch_office_id', 'box_id'], 'required', 'on' => self::SCENARIO_CREATE],
				[['username', 'email', 'auth_key', 'auth_key_test', 'name', 'last_name','status','switch_status','role','branch_office_id', 'box_id'], 'required', 'on' => self::SCENARIO_UPDATE],
			...
			//typed
                ...
                ['email', 'email'],
				[['status', 'switch_status','branch_office_id', 'box_id'], 'integer'],
				[['fileAvatar','avatar','switch_status','created_at', 'updated_at', 'sellers'], 'safe'],
				[['fileAvatar'], 'file', 'extensions'=>'jpg, gif, png, svg, jpeg'],

                // Validación condicional para el campo 'sellers'
                ['sellers', 'required', 'when' => function($model) {
                    return $model->role == 'Facturador';
                }, 'whenClient' => "function (attribute, value) {
                    return $('#user-role').val() == 'Facturador';
                }", 'message' => 'El campo Sellers es obligatorio para el rol facturador'],
    ...
    {
        return $this->hasMany(Adjustment::className(), ['user_id' => 'id']);
    }
    ...
    {
        return $this->hasMany(ItemEntry::className(), ['user_id' => 'id']);
    }
    ...
    /**
     * Función para obtener la sucursal de los usuarios de tipo empleado
     * @return int|null
     */
    public static function getBranchOfficeIdOfActiveUser()
    {
        $model = User::findOne(Yii::$app->user->id);

        return $model->branch_office_id;
    }
     ...
     * @return int|null
     */
    public static function getBoxIdOfActiveUser()
    {
        $model = User::findOne(Yii::$app->user->id);
    ...
    {
        $extract = substr($this->password_reset_token,0,6);
        $upper_extract = strtoupper($extract);

        return $upper_extract;
    }
     ...
     * @return bool
     */
    public static function checkCodeToken($user_id,$code_token)
    {
        $user = self::findOne($user_id);

        if($user)
        {
            if($code_token === $user->getCodeToken())
            {
                return true;
            }
            else
    ...
    public function fields()
    {
        ...
        $fields['branch_office_label'] = function(User $model){
            return isset($model->branch_office_id)? $model->branchOffice->name : '';
        };

        $fields['avatar'] = function(User $model){
            return isset($model->avatar)?  Yii::$app->urlManager->getBaseUrl() . "/" . $model->getImageUrl() : '';
        };

        $fields['status'] = 'status';
        $fields['status_label'] = function(User $model){
            return isset($model->status)?  User::getStatusValue($model->status, true) : '';
        };
     ...
     * @return array
     */
    public static function getSelectMapAgents($check_status=true)
    {
        $array_map = [];
        ...
        $query = $query = User::find()
            ...
            ->where("auth_assignment.item_name = '".User::ROLE_AGENT."'")
        ;
    ...
    public static function getSelectMapAgentsByBoxId($box_id, $check_status=true)
    {
        $array_map = [];
